   ...
   */
	public function updatesite($porteur){
		...
		if($cronone==false){
			...
			/*if(!empty(Yii::app()->params['isp_porteur_name'][$row->porteur]['nombre'])){	*/
			...
			/*}
				...
				$msg.='le porteur '.$row->porteur.'ainsi que ses sites associés n\'existe pas ';
				echo 'r';
			}*/	
			}
	return $msg ;
			...
			foreach ( $dataReader as $row){
			 ...
			 // $username et $email contiennent les username et email de la ligne courante
				\Controller::loadConfigForPorteur($porteur);
			           ...
			           //update databse idmessage
					  /*  $query=IspCompaign::model()->find(array(
					                      
					                      'condition' => 'idcompaign_sf = :idcompaign_sf ',
					                      'params'    => array(':idcompaign_sf' => $row->idcompaign_sf  )
					                  ));*/
   ...
   */
   ...
   /*    $token=Yii::app()->ispAlimenterBD->gettoken($porteur);
		...
		$dataReader=IspCompaign::model()->findAll(array(
                      ...
                      'params'    => array(':messagename' => 'null' ,':porteur' => $porteur  )
                  ));
	   
       foreach ( $dataReader as $row){*/
			 // $username et $email contiennent les username et email de la ligne courante
       	      
			         /*  $messagename=Yii::app()->ispAlimenterBD->getmesagename($token,$row->idmessage,$porteur);*/

			           //update databse idmessage
					   /* $query=IspCompaign::model()->find(array(
					                      
					                      'condition' => 'idmessage = :idmessage ',
					                      'params'    => array(':idmessage' => $row->idmessage  )
					                  ));*/
...
/*foreach ($model->triggername as $triggernamee) { echo $triggernamee->triggername; }
*/		
		return;
  }
