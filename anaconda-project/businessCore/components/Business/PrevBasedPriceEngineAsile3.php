...
class PrevBasedPriceEngineAsile3 extends \Business\PriceEngine
{
	...
	public function getPrice( $SubCampaignReflation )
	{
		...
		if( empty($this->params->prevRefProduct) || !is_object($this->User) )
			throw new \EsoterException( 106, 'Error PrevBasedPriceEngineAsile3' );
		...
		if( $lastInvoice->getItemCount() > 0 )
		{
			...
			$lastStep = $AllInvoice[1]->priceStep;
	
			$priceStep	    = $lastInvoice->lastStepInvoice($lastStep, "asile3", $currentStep, $steepAsile1, $steepAsile2);
