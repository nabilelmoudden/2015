...
class PrevBasedPriceEngineAsile4 extends \Business\PriceEngine
{
	...
	public function getPrice( $SubCampaignReflation )
	{
		...
		if( empty($this->params->prevRefProduct) || !is_object($this->User) )
			throw new \EsoterException( 106, 'Error PrevBasedPriceEngineAsile4' );
		...
		if( $lastInvoice->getItemCount() > 0 )
		{
			...
			$lastStep = $AllInvoice[2]->priceStep;
			
			
			$priceStep	    = $lastInvoice->lastStepInvoice($lastStep, "asile4", $currentStep, $steepAsile1, $steepAsile2);	
