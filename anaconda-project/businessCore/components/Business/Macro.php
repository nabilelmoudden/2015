...
class Macro{
	...
	public function getUrlMacro($file_name, $porteur, $fid_name, $fid_product, $fid_type, $fid_asile_inter, $fid_gp, $fid_s, $fid_de, $fid_sd, $fid_dest, $fid_bs){
		...
		//d : V1 & V2
		$dateofbirth= ( $porteur == 'in_laetizia' || $porteur =='en_aasha' || $porteur =='en_aasha_acq' || $porteur =='en_aasha_in' || $porteur =='ie_laetizia'  || $porteur =='nl_laetizia' )? '&d=[EMV FIELD]DATEOFBIRTH[EMV /FIELD]':'&d=[EMV FIELD]DATEOFBIRTH,dd/MM/yyyy,fr[EMV /FIELD]';
		$url .= $dateofbirth;
		//m: V1 & V2
	...
	public function getUrlFAQ($porteur, $fid_name, $fid_product, $fid_type, $fid_asile_inter,$file_name){
		...
		$siteTxt = ($porteur == 'en_aasha' || $porteur == 'en_aasha_acq'|| $porteur == 'en_aasha_in') ? "asha_site" : "site";
		$siteTxt = ($porteur == 'no_ml' || $porteur == 'it_ml'  || $porteur == 'dk_rmay' ) ? 'SiteInst' : $siteTxt;


		$ifsite = ($porteur == 'es_laetizia' || $porteur == 'en_alisha' || $porteur == 'en_aasha' || $porteur == 'en_aasha_in' || $porteur =='en_aasha_acq' || $porteur =='en_aasha_acq' || $porteur =='pt_rmay' || $porteur =='br_rmay' || $porteur =='es_rmay' || $porteur =='mx_rmay' || $porteur =='cl_rmay' || $porteur =='ar_rmay' || $porteur =='in_laetizia' || $porteur =='ie_laetizia' || $porteur =='pt_laetizia' || $porteur =='br_laetizia' || $porteur =='br_rinalda' || $porteur =='pt_rinalda') ? "&site=[EMV FIELD]SITE[EMV /FIELD]" : "";
        ...
        $url  = ($porteur =='fr_davina'|| $porteur =='no_ml'|| $porteur =='it_ml'|| $porteur =='se_rmay' || $porteur =='fi_lae') ? ($site.'/voyances/index.php/SiteInst/FAQ?') : ($site.'/'.$GLOBALS['porteurUrl'][$porteur]['folder'].'/'.$siteTxt.'/index.php?'.$page.'='.$faq.'.php&');
			
		}

		//site avec url siteinst
		$url .= 'm=[EMV FIELD]EMAIL[EMV /FIELD]'.$ifsite.'&ref=';
		if($porteur == "fr_althea") $porteur = 'alt';
		...
		if($porteur == "tr_laetizia") $porteur = 'tr_lae';
		//Le numero d'asile
