   ...
   protected function parseString( \Twig_Environment $environment, $context, $string ) {
	...
	public function getFunctions()
	{
		return array(
			...
			'getSessionStatus'		    => new Twig_Function_Method( $this, 'getSessionStatus' ),
			'setSessionStatus'		    => new Twig_Function_Method( $this, 'setSessionStatus' ),
	...
	{
		return \Yii::app()->session['status'];
	}

	public function setSessionStatus($var)
	{
		 Yii::app()->session['status'] = $var;
	}
	...
	public function getPrice( $idSubCampaign, $refBatchSelling, $priceStep, $refPricingGrid, $idSite, $parite, $format = true, $default = false )
	{
		\Yii::import( 'ext.PriceHelper' );

		$PriceHelp	= new PriceHelper();
		if($default)				
			$PG			= \Business\DefaultPricingGrid::get( $refBatchSelling, $priceStep, $refPricingGrid, $default);
		else
	...
	public function getPricePariteOld( $idSubCampaign, $refBatchSelling, $priceStep, $refPricingGrid, $idSite, $parite, $format = true ,$default = false )
	{
		\Yii::import( 'ext.PriceHelper' );
		...
		if($default)
		{
			$PG	= $PG_1	= \Business\DefaultPricingGrid::get( $refBatchSelling, $priceStep, $refPricingGrid,$default);
			if(is_object($PG))	
				$PGPrice =  $PG->priceATI;
			else
				$PGPrice = 0;
	...
	public function getPriceParite( $idSubCampaign, $refBatchSelling, $priceStep, $refPricingGrid, $idSite, $parite, $format = true ,$default = false)
	{
		\Yii::import( 'ext.PriceHelper' );
		...
		{
			//BEGIN Ahmed code
			$porteurCheck = Yii::app()->params['porteur']; //$_SESSION['porteur']
			$isMS = ( in_array($porteurCheck, $GLOBALS['porteurMultiSite']) ) ? 1 : 0;
			$defSite = NULL;
			if($isMS==1) $defSite = $GLOBALS['DefaultSite'][$porteurCheck];
			$curSite = \Business\Site::model()->FindByPk($idSite);

			//if ( $isMS == 0 || ( $isMS == 1 && ($idSite == 1 && ($porteurCheck == 'en_aasha' || $porteurCheck == 'en_alisha' || $porteurCheck == 'es_laetizia')) ) ) {
					
			//if ( $isMS == 0 || isset($GLOBALS['DefaultSite'][$porteurCheck]) ) {
			if ( ($isMS == 0) || ($isMS == 1 && ($defSite == $curSite->code) ) ) {
				//Not Ahmed code
				//$PG	= $PG_1	= \Business\DefaultPricingGrid::get( $refBatchSelling, $priceStep, $refPricingGrid,$default);
				...
				//var_dump($idSite, $porteurCheck,$GLOBALS['DefaultSite'][$porteurCheck], \Business\Site::model()->FindByPk($idSite)->code);die();
			//} elseif ( $isMS == 1 && ($idSite > 1 && ($porteurCheck == 'en_aasha' || $porteurCheck == 'en_alisha' || $porteurCheck == 'es_laetizia')) ) {
			} elseif ( $isMS == 1 && ($defSite != $curSite->code) ) {
				$PG_1		= \Business\PricingGrid::get( $idSubCampaign, $refBatchSelling, $priceStep, $refPricingGrid, $id_site);		
				$PG			= \Business\PricingGrid::get( $idSubCampaign, $refBatchSelling, $priceStep, $refPricingGrid, $idSite );
