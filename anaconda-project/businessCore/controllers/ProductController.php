...
class ProductController extends AdminController
{
   ...
   public function actionopenEmails(){
    	$this->layout = "";
    	$this->renderPartial( '//product/macroanaconda');
    	
    
    }  
    //Action qui sert a nous rediriger vers la nouvelle macro anaconda
    
    public function actionMacroAnaconda(){
    	...
    	$fid_en_cours = \Business\T2_projet::loadByref(0);
    
    	// Vérification sur les porteurs multi-site pour afficher le site du pays ou pas sur le dropdownlist
    
    	if(!empty($fid_en_cours->id_projet) AND $this->isMultiSite2() == 1){
    		$drop_fid = CHtml::listData($fid_en_cours->findAll(array('condition'=>'date_shoot>NOW()')), 'id_projet', function($fid_en_cours) {return $fid_en_cours->nomenclature.' ('.$fid_en_cours->site.')'; } );
    	}
    	elseif(!empty($fid_en_cours->id_projet) AND $this->isMultiSite2() != 1)
    	{
    		$drop_fid = CHtml::listData($fid_en_cours->findAll(array('condition'=>'date_shoot>NOW()')), 'id_projet', function($fid_en_cours) {return $fid_en_cours->nomenclature; } );
    	}
    	else
    	{
    		echo $drop_fid = "";
    	...
    	if(Yii::app()->request->getParam('submitForm') !== NULL){
    		...
    		$porteur.=$isacq;
    		$bouton="[EMV INCLUDE]{$GLOBALS['porteurUrl'][$porteur]['bouton1']}[EMV /INCLUDE][EMV LINK]1[EMV /LINK][EMV INCLUDE]{$GLOBALS['porteurUrl'][$porteur]['bouton2']}[EMV /INCLUDE]";
    			
    		//Récupration des variables
    		$fid_name = Yii::app()->request->getParam('fid_name');
    		// Text preheader
    		$fid_preheader 		= Yii::app()->request->getParam('fid_preheader');
    		...
    		$fid_gp = "GP_ANACONDA";
    		$fid_s = "STATUT_ANACONDA";
    		$fid_de = "DE_ANACONDA";
    		$fid_sd = "SD_ANACONDA";
    		$imagesPath = Yii::app()->request->getParam('images_path');
    		$mailPathCheck = false;
    		$imagePathCheck = false;
    		...
    		if (!array_key_exists($porteur,$GLOBALS['porteurUrl'])) {
    			...
    			$this->render( '//product/macroanaconda', array('Router' => $Router, 'porteur' => $_SESSION['porteur'],'drop_fid' => $drop_fid));
    			die();
    		}
    		$macroErrors = "";
    		$required = "";
    		...
    		//check if imagesPath is correct
    		foreach(explode('_', $imagesPath) as $value){
    			if($value == 'mail' || $value == '/mail'){
    				$mailPathCheck = true;
    			}
    			if($value == 'images'){
    				$imagePathCheck = true;
    		...
    		//si le path email et images est incorrect return error
    		if (!$mailPathCheck || !$imagePathCheck || substr_count($imagesPath, '/') <= 1) {
    			$macroErrors .= "le chemin specifié n'est pas conforme<br>";
    		}
    		...
    		// Lecture des images a uploader
    		$imagesMails = CUploadedFile::getInstancesByName('imagesFiles');
    		//create images folder if it doesnt exist
    		if ((!file_exists(realpath(__DIR__ . '/../../..').$imagesPath))) {
    			mkdir((realpath(__DIR__ . '/../../..').$imagesPath), 0775, true);
    		}
    		foreach($imagesMails as $images => $file){
    
    			//move images to images folder.
    			if ((!file_exists(realpath(__DIR__ . '/../../..').$imagesPath.'/'.$file->name))) {
    				if (!$file->saveAs(realpath(__DIR__ . '/../../..').$imagesPath.'/'.$file->name)) {
    					echo "Probléme lors de l'enregistrement de l'image";exit;
    				}
    			}else{
    				continue;
    			}
    		...
    		//declaration d'un tableau qui va contenir les valeurs dynamique
    		$tableContenuDynamique=null;
    			
    		//max des contenues dynamiques à créer
    		$maxContenuDynamique=0;
    		...
    		foreach($excelfiles as $macroFile => $pic){
    			...
    			///ajouter test macro.php sur les porteurs fids et acq
    			$htmlData = $this ->actionGetHtmlCodeAnaconda($fid_border_color,  $fid_preheader,$fid_name,$porteur,$fid_product, strtolower ($pic->name));
    			...
    			if (!copy(dirname(__FILE__).'/../../views/macro/macrosdocs/'.$pic->name, $filenamezip)){
    				die("La copie du fichier a échoué...\n");
    			}
    			...
    			$macroURL = $macro->getUrlMacroAnaconda($pic->name, $porteur, $fid_name, $fid_product, $fid_type, $fid_asile_inter, $fid_gp, $fid_s, $fid_de, $fid_sd);
    			...
    			foreach($paragraphes as $para){
    				...
    				if ($porteur == 'in_laetizia' || $porteur =='en_aasha' || $porteur =='en_aasha_acq' || $porteur =='en_aasha_in' || $porteur =='ie_laetizia'  || $porteur =='nl_laetizia' ) {
    				...
    				$paratext = str_replace("# prénom", $emvFName, $paratext);
    				$paratext = str_replace("#prénom", $emvFName, $paratext);
    				//changement pour prise en compte du bouton
    					
    				$paratext = str_replace("#Bouton", "", $paratext);
    				$paratext = str_replace("# Bouton", "", $paratext);
    				$paratext = str_replace("#bouton", "", $paratext);
    				$paratext = str_replace("# bouton", "", $paratext);
    				...
    				$paratexthtml = str_replace("#Bouton", $bouton, $paratexthtml);
    				$paratexthtml = str_replace("# Bouton", $bouton, $paratexthtml);
    				$paratexthtml = str_replace("#bouton", $bouton, $paratexthtml);
    				$paratexthtml = str_replace("# bouton", $bouton, $paratexthtml);
    				...
    				if (isset($GLOBALS['porteurUrl'][$porteur]['emvsignature']) && $GLOBALS['porteurUrl'][$porteur]['emvsignature'] !== '') {
    					$paratexthtml = preg_replace('@<pr>(.*)#(.*)signature(.*)</pr>@i', $htmlData['bouton']."[EMV /INCLUDE]".$GLOBALS['porteurUrl'][$porteur]['emvsignature']."[EMV /INCLUDE]", $paratexthtml);
    				} else {
    					$paratexthtml = preg_replace('@<pr>(.*)#(.*)signature(.*)</pr>@i', $htmlData['bouton'].$htmlData['imgsignature'], $paratexthtml);
    				}
    				//variable cheque
    				//variable porteur
    			...
    			{
    				$porteurMapp = $GLOBALS ['porteurMap'] [explode ( '_', $porteur ) [0] . '_' . explode ( '_', $porteur ) [1] . '_' . explode ( '_', $porteur ) [2]];
    			}
    			else
    			{
    			...
    			//recuperer objet d'email
    			$entete_test = str_replace('-','',$htmlData['afterObject']);
    
    			$html2 = substr($entete_test,5,strlen($entete_test)-5)."\n".wordwrap($allparatext, 80, "\n\t").$htmlData['footertext'];
    
    			$html2.=$html;
    
    			/********************************************************************* Génération des emails via l'API *******************************************************************************/
    
    			/********************************************* Connexion à l'API ***********************************************/
    
    			$GLOBALS2['DefaultSite2'] = array(
    					'en_alisha'		=> 'in',
    					'en_aasha'		=> 'uk',
    					'es_laetizia'	=> 'es',
    			);
    			if( $fid_acq == 'in' )
    			{
    				...
    				$PORTEURE = $_SESSION['porteur'];
    					
    				$_SESSION['porteur'] = 'in_'.(explode('_',$_SESSION['porteur'])[0]);
    
    			}
    			elseif( !empty( $GLOBALS2['DefaultSite2'][$_SESSION['porteur']] ) )
    			{
    				$PORTEURE = $_SESSION['porteur'];
    					
    				$_SESSION['porteur'] = $GLOBALS2['DefaultSite2'][$_SESSION['porteur']].'_'.(explode('_',$_SESSION['porteur'])[0]);
    			}
    
    			elseif( !empty($fid_site) )
    			{
    				$PORTEURE = $_SESSION['porteur'];
    					
    				$_SESSION['porteur'] = $fid_site.'_'.(explode('_',$_SESSION['porteur'])[0]);
    			...
    			if( $fid_acq == 'fid'  )
    			{
    				...
    				$class_api = new Class_API($cmd_wdsl_fid,$cmd_login_fid,$cmd_pwd_fid,$cmd_key_fid);
    			}
    			else
    			{
    			...
    			// Récupere  le nom du porteur
    
    			$porteur_name = $class_api->getNomPorteur($token);
    
    
    
    			//récuperer la langue du porteur
    			$porteur_langue = $class_api->getLangue($token);
    
    			//recupere email sender
    			$mail = '[EMV INCLUDE]'.$class_api->mail_sender($token).'[EMV /INCLUDE]';
    
    			/******************************************************* partie [j+X]*****************************************/
    			if (isset ( explode ( '_', $porteur ) [2] )){
    				$urldynamique = $GLOBALS['porteurDNS'][explode('_',$porteur)[0].'_'.explode('_',$porteur)[1].'_'.explode('_',$porteur)[2]];
    				$porteurMapp = $GLOBALS['porteurMap'][explode('_',$porteur)[0].'_'.explode('_',$porteur)[1].'_'.explode('_',$porteur)[2]];
    			}
    			else{
    				$urldynamique = $GLOBALS['porteurDNS'][explode('_',$porteur)[0].'_'.explode('_',$porteur)[1]];
    				$porteurMapp = $GLOBALS['porteurMap'][explode('_',$porteur)[0].'_'.explode('_',$porteur)[1]];
    			}
    				
    
    			/******************************************************partie objet*****************************************/
    			while( preg_match( '#\[J\+[0-9][0-9]?\]#' , $textObjets[1] ) )
    			{
    					
    				if( $maxContenuDynamique > 20 )
    				{
    					break;
    				}
    				...
    				$number = $class_api->get_string_between($textObjets[1],'[J+',']');
    					
    				$exist = false;
    
    				if( (string)(int)$number == $number )
    				{
    					if ( isset($tableContenuDynamique['[J+'.$number.']']) )
    					{
    							
    						$exist = true;
    							
    						$id_contenudynamique = $tableContenuDynamique['[J+'.$number.']'];
    					}
    				}
    				...
    				if( !$exist )
    				{
    					...
    					$contentD = "[EMV EXTERNAL][EMV CACHED]false[EMV /CACHED][EMV URLNAME]http://$urldynamique/$porteurMapp/fonctions_gen/dateComplet.php?rt=returnDt&sd=%2B$number&language=$porteur_langue&sdEmv=[EMV FIELD]$fid_sd"."[EMV /FIELD][EMV /URLNAME][EMV /EXTERNAL]";
    
    					$dynamiqueName = "[j+".$number."] ".$fid_name;
    
    					$id_contenudynamique = $class_api->createContenuDynamique($token,$dynamiqueName,$descriptionContenuDynamique,$contentD,$contentTypeD);
    
    					$tableContenuDynamique['[J+'.$number.']'] = $id_contenudynamique;
    				}
    
    				$textObjets[1] = str_replace('[J+'.$number.']','[EMV INCLUDE]'.$id_contenudynamique.'[EMV /INCLUDE]',$textObjets[1]);
    			...
    			while( preg_match('#\[J\+[0-9][0-9]?\]#', $html2) )
    			{
    				...
    				$number = $class_api->get_string_between($html2,'[J+',']');
    				...
    				$html2=str_replace('[J+'.$number.']','[EMV INCLUDE]'.$id_contenudynamique.'[EMV /INCLUDE]',$html2);
    				$maxContenuDynamique++;
    			}
    			...
    			// Création de l'email au niveau de SF
    			$class_api->createMessageEmail($token,$fileNameZip[0],$textObjets[1],$porteur_name,$html2,$porteur_name,$mail,$isBounceback,$description,$encoding,$hotmailUnsubFlg,$hotmailUnsubUrl,$type,$macroURL,$FAQURL);
    
    
    
    			/********************************************************************* Fin Génération des emails via l'API *******************************************************************************/
    		...
    		//transfer all generated html mails to porteur mail folder.
    		$explodedImagesPath = explode('/',$imagesPath);
    		if(array_key_exists(1,$explodedImagesPath)){
    
    			$this->xcopy($folder.'/Serveur', realpath(__DIR__ . '/../../../').'/'.$explodedImagesPath[1].'/'.$fid_name.$ct, 0777);
    		}
    			
    		Yii::app()->user->setFlash( "success", "<b>Les fichiers sont générés dans le dossier : ".$this->portViewDir."generated/$porteur/$fid_name$ct</b>" );
    		$this->render( '//product/macroanaconda', array('Router' => $Router, 'porteur' => $_SESSION['porteur'],'drop_fid' => $drop_fid));
	...
	public function actionCampaign(){
		...
		//  Valider la crÃƒÆ’Ã‚Â©ation du CDC
		...
		if( Yii::app()->request->getParam('valide') )
		{
			...
			if( $toValide !== NULL )
			{
				...
				if( $toValide->save() )
				{
					...
					$porteur    = Yii::app()->session['porteur'];
					$msg  = '<div style=" font-size:15px;"><u>'.$porteur.'('.$toValide->num.')</u> : le service <b>Marketing</b> a effectuÃƒÆ’Ã‚Â© la livraison du CDC de la campagne(<b>'.$toValide->num.'</b>). </div>';
		...
		//  Valider le contrÃƒÆ’Ã‚Â´le du CDC
		...
		if( Yii::app()->request->getParam('controle') )
		{
			...
			if( $toValide !== NULL )
			{
				...
				if( $toValide->save() )
				{
					$suiviPlanification  = $toValide->SuiviPlanification($toValide->id);
					$porteur    = Yii::app()->session['porteur'];
					$msg  = '<div style=" font-size:15px;"><u>'.$porteur.'('.$toValide->num.')</u> : le service <b>qualitÃƒÆ’Ã‚Â©</b> a effectuÃƒÆ’Ã‚Â© le contrÃƒÆ’Ã‚Â´le CDC de la campagne(<b>'.$toValide->num.'</b>). </div>';
					$msg .= $suiviPlanification;
					$expediteur = "Suivi Planification(".$porteur.")";
					$sendAlert  = \MailHelper::sendMail( $toValide->adminMails[$porteur], $porteur, 'Maj Suivi de Planification', $msg );
		...
		//  Valider le contrÃƒÆ’Ã‚Â´le du Projet
		...
		if( Yii::app()->request->getParam('controle_project') )
		{
			...
			if( $toValide !== NULL )
			{
				...
				if( $toValide->save() )
				{
					$suiviPlanification  = $toValide->SuiviPlanification($toValide->id);
					$porteur    = Yii::app()->session['porteur'];
					$msg  = '<div style=" font-size:15px;"><u>'.$porteur.'('.$toValide->num.')</u> : le service <b>qualitÃƒÆ’Ã‚Â©</b> a effectuÃƒÆ’Ã‚Â© le contrÃƒÆ’Ã‚Â´le de la campagne(<b>'.$toValide->num.'</b>). </div>';
					$msg .= $suiviPlanification;
					$expediteur = "Suivi Planification(".$porteur.")";
					$sendAlert  = \MailHelper::sendMail( $toValide->adminMails[$porteur], $porteur, 'Maj Suivi de Planification', $msg );
		...
		if( Yii::app()->request->getParam('developper') )
		{
			...
			if( $toValide !== NULL )
			{
				...
				if( $toValide->save() )
				{
					$suiviPlanification  = $toValide->SuiviPlanification($toValide->id);
					$porteur    = Yii::app()->session['porteur'];
					$msg  = '<div style=" font-size:15px;"><u>'.$porteur.'('.$toValide->num.')</u> : le service <b>IT</b> a effectuÃƒÆ’Ã‚Â© la crÃƒÆ’Ã‚Â©ation du campagne (<b>'.$toValide->num.'</b>).</div>';
					$msg .= $suiviPlanification;
					$expediteur = "Suivi Planification(".$porteur.")";
					$sendAlert  = \MailHelper::sendMail( $toValide->adminMails[$porteur], $porteur, 'Maj Suivi de Planification', $msg );
		...
		if( Yii::app()->request->getParam('valide_project') )
		{
			...
			if( $toValide !== NULL )
			{
				...
				if( $toValide->save() )
				{
					$suiviPlanification  = $toValide->SuiviPlanification($toValide->id);
					$porteur    = Yii::app()->session['porteur'];
					$msg  = '<div style=" font-size:15px;"><u>'.$porteur.'('.$toValide->num.')</u> : le service <b>Marcketing</b> a effectuÃƒÆ’Ã‚Â© la validation du campagne (<b>'.$toValide->num.'</b>).</div>';
					$msg .= $suiviPlanification;
					$expediteur = "Suivi Planification(".$porteur.")";
					$sendAlert  = \MailHelper::sendMail( $toValide->adminMails[$porteur], $porteur, 'Maj Suivi de Planification', $msg );
	...
	public function actionExcel()
	{
		...
		if(isset($_POST['tblexcel'])){
			...
			Yii::app()->user->setFlash( "error", Yii::t( 'common', 'Erreur lors de la rÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration du tableau ÃƒÆ’  exporter' ) );
	}
	public function actionExcel_v2()
	{
		...
		$old_porteur = Yii::app()->session['porteur'];
		
		$objPHPExcel = new PHPExcel();

		
		if(isset($_POST['tblexcel']) OR 1){
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B3', 'Suivi KIM Projets: ');
			...
			$logo = 'images/logoKIM.png'; // Provide path to your logo file
			...
			$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('B8', 'Porteur')
				->setCellValue('C8', 'Pays')
				->setCellValue('D8', 'NÃƒâ€šÃ‚Â°Fid')
				->setCellValue('E8', 'Label')
				->setCellValue('F8', 'Type projet')
				->setCellValue('G8', 'Date Shoot')
				->setCellValue('H8', 'Date rÃƒÆ’Ã‚Â©ception CDC - PrÃƒÆ’Ã‚Â©visionnelle')
				->setCellValue('I8', 'Date rÃƒÆ’Ã‚Â©ception CDC - RÃƒÆ’Ã‚Â©elle')
				->setCellValue('J8', 'Validation du CDC - PrÃƒÆ’Ã‚Â©visionnelle')
				->setCellValue('K8', 'Validation du CDC - RÃƒÆ’Ã‚Â©elle')
				//->setCellValue('J8', 'Lancement IT')
				->setCellValue('L8', 'Fin intÃƒÆ’Ã‚Â©gration IT - PrÃƒÆ’Ã‚Â©visionnelle')
				->setCellValue('M8', 'Fin intÃƒÆ’Ã‚Â©gration IT - RÃƒÆ’Ã‚Â©elle')
				->setCellValue('N8', 'Validation QualitÃƒÆ’Ã‚Â© - PrÃƒÆ’Ã‚Â©visionnelle')
				->setCellValue('O8', 'Validation QualitÃƒÆ’Ã‚Â© - RÃƒÆ’Ã‚Â©elle')
				->setCellValue('P8', 'Livraison Marketing - PrÃƒÆ’Ã‚Â©visionnelle')
				->setCellValue('Q8', 'Livraison Marketing - RÃƒÆ’Ã‚Â©elle')
				->setCellValue('R8', 'Commentaire');

			$objPHPExcel->getActiveSheet()->getStyle('B8:R8')->getFont()->setBold(true)
				->getColor()
				->setRGB('FFFFFF');
			...
			foreach ($GLOBALS['porteurMap'] as $porteur => $folder) {
				if(in_array($folder, $prev_folders))
					continue;

				$prev_folders[] = $folder;

				$this->loadConfigForPorteur2( $porteur );

				$Camp= new \Business\Campaign('search');

				$suivi_plan_site = $Camp->SuiviPlanification_v2($porteur);
				...
				if(strtolower($p) == 'rmay' || strtolower($p) == 'ml')
					$p = 'Mona Luisa';

				if(count($sheet)>0){
					$objPHPExcel->getActiveSheet()->setCellValueExplicit('B'.$rowID, ucfirst($p), PHPExcel_Cell_DataType::TYPE_STRING);
					$objPHPExcel->getActiveSheet()->getStyle('B'.$rowID)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$objPHPExcel->getActiveSheet()->getStyle('B'.$rowID)
						->getAlignment()
						->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
					$objPHPExcel->getActiveSheet()->getStyle('B'.$rowID)
						->getAlignment()
				...
				foreach($sheet as $rowArray) {
					...
					if (!isset($merge[$index_merge])) {
						$merge[$index_merge] = array();
					}
					array_push($merge[$index_merge], $rowID);
					$index_merge++;
			...
			$this->loadConfigForPorteur2( $old_porteur );

			$objPHPExcel->getActiveSheet()->setAutoFilter('C8:Q'.$rowID);
		}
		else
			Yii::app()->user->setFlash( "error", Yii::t( 'common', 'Erreur lors de la rÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration du tableau ÃƒÆ’  exporter' ) );
		...
		$objPHPExcel->getActiveSheet()->getStyle('B10')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('50d092');
		$objPHPExcel->getActiveSheet()->getStyle('B11')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('ff0000');
		$objPHPExcel->getActiveSheet()->getStyle('B12')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('d9d9d9');
		$objPHPExcel->getActiveSheet()->getStyle('B13')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('ffff00');

		$objPHPExcel->setActiveSheetIndex(1)
			->setCellValue('C10', 'RÃƒÆ’Ã‚Â©ception dans les dÃƒÆ’Ã‚Â©lais ')
			->setCellValue('C11', 'RÃƒÆ’Ã‚Â©ception en retard')
			->setCellValue('C12', 'RÃƒÆ’Ã‚Â©ception prÃƒÆ’Ã‚Â©vue')
			->setCellValue('C13', 'RÃƒÆ’Ã‚Â©ception avancÃƒÆ’Ã‚Â©e');
	...
	public function actionCampaignShow(){
		...
		if( Yii::app()->request->getParam( 'Business\Campaign' ) !== NULL )
		{
			...
			// date prÃƒÆ’Ã‚Â©visionnelle de lancement de dev de L'IT
			...
			//BEGIN Ahmed code
			$porteurCheck = Yii::app()->params['porteur'];
			if ($this->isMultiSite2() == 1 && !$upCamp) {
				$Camp->idSite = Yii::app()->request->getParam( 'Business\Campaign' )['idSite'];
			}			
			//END Ahmed code
			
			//Make fid accessible for IT service for IT account
			if(empty($Camp->projectStatus))
				$Camp->projectStatus = \Business\Campaign::PROJECT_CONTROL;
			...
			if( $Camp->save() )
			{
				if(isset($suiviPlanification)){
					...
					$msg  = '<div style="font-size: 15px;"><b><u>'.$porteur.'('.$Camp->num.')</u></b> : Le service <b>IT</b> a mis ÃƒÆ’  jour la date de livraison prÃƒÆ’Ã‚Â©visionnelle de projet (<b>'.$Camp->num.'<b>).</div>';
					$msg .= $suiviPlanification;
					$expediteur = "Suivi Planification(".$porteur.")";
	    ...
	    foreach($sites as $site) {
				$allSites[$site->id] = $site->code;
		}
		//END Ahmed code

		$this->renderPartial( '//product/campaignShow', array( 'Camp' => $Camp, 'allSites'=>$allSites ) );
	}
	...
	/*  Code ajoutÃƒÆ’Ã‚Â© par Samir: 23/09/2014 */
...
//Badr Yassine -- passer a la lecture du fichier
	...
	public function actionSubCampaignMark(){
		...
		$this->renderPartial( '//product/subCampaignMark', array( 'Camp' => $Camp, 'Search' => $Search, 'RoleUser' => $role_user) );
	}
	...
	public function actionProductShow(){
		...
		if( Yii::app()->request->getParam( 'Business\Product' ) !== NULL )
		{
			...
			$priceModel_Prod = ($Prod->priceModel == 'prevBasedAsile2' || $Prod->priceModel == 'prevBasedAsile3' || $Prod->priceModel == 'prevBasedAsile4' || $Prod->priceModel == 'prevBasedAsile2Althea' || $Prod->priceModel == 'prevBasedAlthea' )? 'prevBased' : $Prod->priceModel;
		...
		if( Yii::app()->request->getParam( 'Business\Campaign' ) !== NULL )
		{
			...
			if( $Camp->save() ){
				if(isset($suiviPlanification)){
					$suiviPlanification  = $Camp->SuiviPlanification($Camp->id);
					$porteur    = Yii::app()->session['porteur'];
					$msg  = '<div style="font-size: 15px;"><b><u>'.$porteur.'('.$Camp->num.')</u></b> : le service qualitÃƒÆ’Ã‚Â© a mis ÃƒÆ’  jour la date de livraison prÃƒÆ’Ã‚Â©visionnelle de projet (<b>'.$Camp->num.'<b>).</div>';
					$msg .= $suiviPlanification;
					$expediteur = "Suivi Planification(".$porteur.")";
					$sendAlert  = \MailHelper::sendMail( $Camp->adminMails[$porteur], $porteur, 'Maj Suivi de Planification', $msg );
				}
	...
	public function actionCampaignCreateAdaptMark(){
		...
		//Begin Ahmed Code
		$idSite = $lacamp->idSite;
	    foreach($sites as $site) {
	      	if($site->id != $idSite && !in_array($site->code, $listSitesCrees))
	        	$listSites[] = $site->code;
	    }
	    //End Ahmed Code
	...
	public function actionControleAdapt(){
		...
		foreach($camps AS $camp){
			...
			if(empty($camp->num)){
				$status = false;
				$controle[$camp->site]['camp'] = 'Le numÃƒÆ’Ã‚Â©ro de la campagne d\'adaptation est vide.';
			}
			...
			foreach($subs AS $sub){
				if(empty($sub->link_mail)){
					$status = false;
					$controle[$camp->site]['prod'][$i]['link_mail'] = 'Aucune adaptation de texte pour les Links Mail n\'a ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© mentionnÃƒÆ’Ã‚Â©e.';
				}

				if(empty($sub->page)){
					$status = false;
					$controle[$camp->site]['prod'][$i]['page'] = 'Aucune adaptation de texte pour les pages web n\'a ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© mentionnÃƒÆ’Ã‚Â©e.';
				}

				if(empty($sub->nb)){
					$status = false;
					$controle[$camp->site]['prod'][$i]['nb'] = 'Aucune note (N.B.) d\'adaptation de texte n\'a ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© mentionnÃƒÆ’Ã‚Â©e.';
				}
				$i++;
			}
	...
	public function actionGenerateCDCAdapt(){
		...
		//Begin Ahmed code
		$codeSiteCur = \Business\Site::model()->FindByPk($Camp->idSite)->code;
		$document->setValue('leporteur', $porteur);
		$document->setValue('idcampgsource', $Camp->num);
		$document->setValue('porteurSource', $porteurCur. ' '.$codeSiteCur);
		//End Ahmed code
	...
	//BEGIN Ahmed code
	public function isMultiSite2(){
		if( in_array(Yii::app()->params['porteur'], $GLOBALS['porteurMultiSite']) )
			return true;
		else
			return false;
	}
...
// ====================================================================================================
	...
	public function actionCampaignShowMark(){
		...
		if( $id !== NULL && ! empty($id) )
		{
			...
			$nbprodcts = \Business\SubCampaign::model()->findAllByAttributes(array( 'idCampaign' => $id ));		
			$nbpr = count($nbprodcts);
			...
			// ******************************************** RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©rÃƒÆ’Ã‚Â© le premier produit s'il est crÃƒÆ’Ã‚Â©ÃƒÆ’Ã‚Â© * //
		...
		// ******************************************** Mise a jour ou crÃƒÆ’Ã‚Â©ation de la campagne -- rÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration des elements saisis * ///
		if( Yii::app()->request->getParam( 'Business\Campaign' ) !== NULL ){
			// Log l'action courante
			$this->logAction( new CEvent($this, array( 'action' => \Business\Log::ACTION_ADMIN)));
			...
			$Camp->type_projet         = Yii::app()->request->getParam( 'Business\Campaign' )['type_projet'];
			...
			if( empty($Camp->ref) )
				die('erreur: La rÃƒÆ’Ã‚Â©fÃƒÆ’Ã‚Â©rence de la Campagne est obligatoire !');
			if( empty($Camp->titre_stat) )
				die('erreur: Le Titre Stats de la Campagne est obligatoire');

			// ******************************************** Enregistrement des donnÃƒÆ’Ã‚Â©es de la Campagne * //
			if( $Camp->save() ){
				...
				for ($i=0; $i<4; $i++){
					...
					if(empty($dt) || $dt == "" || $dt == " ")
						$dt = "0000-00-00";
					// ******************************************** RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration des elements saisis - 1er Compte * //
					...
					if($nbcpt == 2){
						...
						if(empty($dt) || $dt == "" || $dt == " ")
							$dt = "0000-00-00";
						// ******************************************** RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration des elements saisis - FID* //
				...
				if($msgerr == ""){
					if(isset($suiviPlanification)){
						$suiviPlanification  = $Camp->SuiviPlanification($Camp->id);
						$porteur    = Yii::app()->session['porteur'];
						$msg  = '<div style=" font-size:15px;"><u>'.$porteur.'('.$Camp->num.')</u> : le service <b>Marketing</b> a effectuÃƒÆ’Ã‚Â© une mise a jour du date de livraison prÃƒÆ’Ã‚Â©visionnelle du CDC de la campagne(<b>'.$Camp->num.'</b>). </div>';
						$msg .= $suiviPlanification;
						$expediteur = "Suivi Planification(".$porteur.")";
						if(!isset($Camp->adminMails[$porteur]))
							$sendAlert  = \MailHelper::sendMail( array('younes.elbague.ki@gmail.com','jalal.bensaad.ki@gmail.com', 'selmi.younes.ki@gmail.com'), $porteur, 'Maj Suivi de Planification', $msg );
						else
							$sendAlert  = \MailHelper::sendMail( $Camp->adminMails[$porteur], $porteur, 'Maj Suivi de Planification', $msg );
					}
					Yii::app()->user->setFlash( "success", Yii::t( 'common', 'updateOK' ) );
					...
					Yii::app()->user->setFlash( "success", Yii::t( 'common', 'Campagne enregistrÃƒÆ’Ã‚Â©e' ) );
				}
			}else{
				...
				if(!empty($searchLabel))
					die('erreur: Label existe dÃƒÆ’Ã‚Â©jÃƒÆ’ , veuillez choisir un autre label svp!');
				elseif(!empty($searchRef))
					die('erreur: RÃƒÆ’Ã‚Â©fÃƒÆ’Ã‚Â©rence existe dÃƒÆ’Ã‚Â©jÃƒÆ’ , veuillez choisir une autre rÃƒÆ’Ã‚Â©fÃƒÆ’Ã‚Â©rence svp!');
				else
		...
		$this->renderPartial( '//product/campaignShowMark', array( 'Camp' => $Camp,'nbpr' => $nbpr, 'CampShoot' => $CampShoot, 'nbcpt' => $nbcpt, 'porteurs'=>$lalistedesporteursfr, 'allSites'=>$allSites, 'currentPorteur'=>$porteur) );
	...
	public function actionProductShowMark(){
		...
		// Update				if( Yii::app()->request->getParam( 'idCamp' ) !== NULL ){
		if( Yii::app()->request->getParam( 'id' ) > 0 ){					
			if( !($Sub = \Business\SubCampaign::load( Yii::app()->request->getParam( 'id' ) )) )		
				return false;		
			$Prod	= $Sub->Product;		
		...
		else if( Yii::app()->request->getParam( 'idCamp' ) !== NULL ){
		...
		if( Yii::app()->request->getParam( 'Business\Product' ) !== NULL )
		{
			...
			$porteur = Yii::app()->params['porteur'];
			// ******************************************** RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration des champs du formulaire * //
			$Sub->attributes	= Yii::app()->request->getParam( 'Business\SubCampaign' );
			$Prod->attributes	= Yii::app()->request->getParam( 'Business\Product' );
			...
			if($Sub->id != NULL && ($Prod->price_step != Yii::app()->request->getParam( 'Business\Product' )['price_step']))					
			{		
				$codPorteur = substr($porteur, 0, 2);		
				if($codPorteur == 'en')		
					$codPorteur = 'in';		
				$site = \Business\Site::loadByCode($codPorteur);		
				$priceGrids = \Business\PricingGrid::getBySubCampaignAndSite($Sub->id,$site->id);		
				foreach ($priceGrids as $priceGrid):		
					if(method_exists($priceGrid, 'delete'))		
					{		
						$priceGrid->delete() ;		
					}		
				endforeach;		
			...
			$Prod->nbrelance = Yii::app()->request->getParam( 'Business\Product' )['nbrelance'];
			...
			if( empty($Prod->price_step) )
				die('erreur: Il faut crÃƒÆ’Ã‚Â©er au moins une "price step" !');
			...
			if( $Sub->idProduct > 0 ) {
				if( $Sub->save() && $Prod->save() )
					...
					die;*/
					die('erreur: Impossible de CrÃƒÆ’Ã‚Â©er la sub campaign et le produit !');
					Yii::app()->user->setFlash( "error", Yii::t( 'common', 'updateNOK' ) );
				}
			}else{
				...
				if( $Prod->save() ) {
					...
					if( $Sub->save() )
						...
						die('erreur: Impossible de CrÃƒÆ’Ã‚Â©er la sub campaign');
						Yii::app()->user->setFlash( "error", 'Erreur lors de la crÃƒÆ’Ã‚Â©ation du SubID. Le produit a ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© supprimÃƒÆ’Ã‚Â©. Veuillez rÃƒÆ’Ã‚Â©essayer' );
						return false;
					}
				}else{
					print_r($Prod->attributes);
					die('erreur: Impossible de CrÃƒÆ’Ã‚Â©er le produit');
					Yii::app()->user->setFlash( "error", "Erreur Lors de la crÃƒÆ’Ã‚Â©ation du Produit. Veuillez rÃƒÆ’Ã‚Â©essayer ou contacter l'administrateur" );
					return false;
				}
			}
		}else{
		...
		/***Test if product has pricingGrid ****/
		$idPrincingGrid = 0;
		if(!empty($Sub->PricingGrid))
			$idPrincingGrid = 1;
		/****************************************/
		$this->renderPartial( '//product/productShowMark', array( 'Sub' => $Sub, 'Prod' => $Prod, 'Camp'=>$Camp, 'lPPSet' => $PPSet->findAll(), 'lSub' => $lSub , 'PG' => $idPrincingGrid) );
	}
	...
	public function actionPricingGridMark(){
		...
		// ******************************************** RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©rer SubCamp * //
		if( Yii::app()->request->getParam( 'id' ) !== NULL ){
			if( !($Sub = \Business\SubCampaign::load( Yii::app()->request->getParam( 'id' ) )) )
				return false;
		}
		else{
			Yii::app()->user->setFlash( "error", 'Erreur lors de la rÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration du SubCampaign !' );
			return false;
		}
	

		$priceStep = $Sub->Product->price_step;	
		...
		if( Yii::app()->request->getParam( 'GP' ) !== NULL )
		{
			...
			//Begin Ahmed code
			/*if ($this->isMultiSite() == 1) {

				if( ($Sub = \Business\SubCampaign::load( Yii::app()->request->getParam('id') ) ) ) {
					$idCampaign = $Sub->idCampaign;
					if( ($Camp = \Business\Campaign::load($idCampaign) ) ) {

						$site = \Business\Site::model()->FindByPk($Camp->idSite);
					}
				}
			} else {
				$site = \Business\Site::load($idSite);
		...
		//Begin Ahmed code
		if ($this->isMultiSite2() == 1) {
			if( ($Sub = \Business\SubCampaign::load( Yii::app()->request->getParam('id') ) ) ) {
				$idCampaign = $Sub->idCampaign;
				if( ($Camp = \Business\Campaign::load($idCampaign) ) ) {
					$sites = \Business\Site::model()->FindByPk($Camp->idSite);
				}
			}
		} else {
			if(isset($GLOBALS['DefaultSite'][$porteur])){
				...
				$codPorteur = substr($porteur, 0, 2);
				$sites = \Business\Site::loadByCode($codPorteur);
			}
		}
		//End Ahmed code
		...
		$tab[ $sites->country ] = array(
				'id'	=> $sites->id,
				'ajax'	=> $this->createAbsoluteUrl( '//product/getPricingGridMark', array( 'idSite' =>  $sites->id, 'idSub' => $Sub->id, 'parite' => $parite->id_parite, 'priceStep' => $priceStep,'nbsites'=>1 ) )
		);
		...
		$this->renderPartial( '//product/pricingGridMark', array( 'Sub' => $Sub, 'tab' => $tab, 'nbsites'=>1 ) );
	...
	public function actionGetPricingGridMark(){
		...
		if ($this->isMultiSite2() == 1) {
			$porteurCheck = Yii::app()->params['porteur'];
			$defSite = $GLOBALS['DefaultSite'][$porteurCheck];
			$curSite = \Business\Site::model()->FindByPk($idSite);
		}
		//End Ahmed code

		//For new porteurs which do not contain table defaultPricingGrid
		if(empty($Sub->PricingGrid) && (($Sub->Product->price_step == 3 && $position == 2) || ($Sub->Product->price_step == 5 && $position == 1)))
		{
			try{
				$defaultPrincingGrid = \Business\DefaultPricingGrid::loadByNbrProduct($position);
				foreach( $defaultPrincingGrid as $GP )
				{
					$data[ $GP->priceStep ][] = array(
							'bs' => $GP->refBatchSelling,
							'gp' => $GP->refPricingGrid,
					...
					if ( $this->isMultiSite2() == 0 || ($this->isMultiSite2() == 1 && ($defSite == $curSite->code)) ) {
		...
		$this->renderPartial( '//product/getPricingGrid_Mark', array( 'Sub' => $Sub, 'idSite' => $idSite, 'parite' => $parite->parite, 'data' => $data, 'data2' => $data2, 'nbRelance' => $nbRelance, 'nbGP' => MAX_GP, 'nbBS' => MAX_BS,'position' => $position, 'nbsites'=>1) );
...
//exit;
		if( is_file($confFile) ) {
			...
			if(isset($GLOBALS['DefaultSite'][\Yii::app()->params['porteur']])){
				...
				if(count($porteurTemp) == 3){
					...
					//BEGIN Ahmed code
					$idCampaign = Yii::app()->request->getParam('idCamp');
					if( ($Camp = \Business\Campaign::load($idCampaign) ) ) {
						$sites = \Business\Site::model()->FindByPk($Camp->idSite);
					}
					$porteur = strtoupper($sites->code).' '.$porteurTemp[0];
					//END Ahmed code
				}
			}else{
				$porteurTemp = explode(' ',$globConf['name']);
		...
		/**VÃƒÆ’Ã‚Â©rifier si au moins un des produits est crÃƒÆ’Ã‚Â©ÃƒÆ’Ã‚Â© */
		...
		if(isset($subcs[0]->Product)){
			...
			//End Ahmed code
			$PG1s = \Business\PricingGrid::model()->findAllByAttributes( array( 'idSubCampaign' => $subcs[0]->id, 'idSite' => $sites->id ), array( 'order'=>'idSite' ) );
			...
			if($nbprods == 2){
				...
				$PG2s = \Business\PricingGrid::model()->findAllByAttributes( array( 'idSubCampaign' => $subcs[1]->id, 'idSite' => $sites->id ), array( 'order'=>'idSite' ) );
				foreach ($PG2s as $x){
			...
			$nbrel = $subcs[0]->Product->nbrelance;
			if(is_Null($nbrel))
					$nbrel = 1;
			for ($i=0; $i <4 ; $i++)
				$document->setValue('nbrel'.($i+1), $nbrel+($i*$nbrel));
			...
			if($nbprods == 2){
				...
				$desc2 = $subcs[1]->Product->description;
				...
				//set value of number between relances for second product
				$nbrel = $subcs[1]->Product->nbrelance;
				if(is_Null($nbrel))
					$nbrel = 2;
				for ($i=0; $i <2 ; $i++)
					$document->setValue('p2rel'.($i+1), $nbrel+($i*$nbrel)+1);
			...
			$paymentMode = 'chÃƒÆ’Ã‚Â¨que';		
			$abvPayment = 'CHQ';		
			if($_SESSION['porteur'] == 'br_laetizia' || $_SESSION['porteur'] == 'br_rmay' || $_SESSION['porteur'] == 'br_rinalda')		
			{		
					$paymentMode = 'Boleto';		
					$abvPayment = 'BOL';		
			}		
			if($_SESSION['porteur'] == 'pt_laetizia' || $_SESSION['porteur'] == 'pt_rmay' || $_SESSION['porteur'] == 'pt_rinalda')		
			{		
				$paymentMode = 'Multibanco';		
				$abvPayment = 'MB';		
			}		
			$document->setValue('payment',$paymentMode);		
			$document->setValue('abvpayment',$abvPayment);
			// ******************************************** Save File * //
		...
		// ****** Si aucun produit n'est encore crÃƒÆ’Ã‚Â©ÃƒÆ’Ã‚Â© * /
		else
			Yii::app()->user->setFlash( "error", Yii::t( 'common', "Vous devez crÃƒÆ’Ã‚Â©er d'abord crÃƒÆ’Ã‚Â©er les produits de cette Campagne !" ) );
...
////////code html pour macro
	public function actionGetHtmlCode($fid_border_color,  $fid_preheader, $nomenclature,$porteur,$prod){
		...
		$error = '';
		$allparatext='';
		/* start declaration text prÃƒÆ’Ã‚Â© header */
		$order  		= array("\r\n", "\n", "\r");
		$html_preheader = str_replace($order, "<br/>", $fid_preheader);
		/* encode html */
		$html_preheader = htmlentities($html_preheader);
		$html_preheader = htmlspecialchars_decode($html_preheader);	
		/* encode special chars */
		$html_preheader = str_replace(
										array('Ãƒï¿½Ã¯Â¿Â½','Ãƒï¿½Ã¢â‚¬Å¡','Ãƒâ€žÃ…Â¾','Ãƒâ€žÃ…Â¸','Ãƒâ€žÃ‚Â±','Ãƒâ€žÃ‚Â°','Ãƒâ€¦Ã…Â¾','Ãƒâ€¦Ã…Â¸','Ãƒâ€¦Ã‚Â¼','Ãƒâ€¦Ã‚Â»' ,'Ãƒâ€¦Ã‚Âº', 'Ãƒâ€¦Ã‚Â¹', 'Ãƒâ€¦Ã¢â‚¬Âº', 'Ãƒâ€¦Ã…Â¡', 'Ãƒâ€¦Ã¢â‚¬Å¾', 'Ãƒâ€¦Ã†â€™', 'Ãƒâ€¦Ã¢â‚¬Å¡', 'Ãƒâ€¦Ã¯Â¿Â½', 'Ãƒâ€žÃ¢â‚¬Â¡', 'Ãƒâ€žÃ¢â‚¬Â ','ÃƒÆ’Ã‚Â³','ÃƒÆ’Ã¢â‚¬Å“','Ãƒâ€žÃ¢â€žÂ¢','Ãƒâ€žÃ‹Å“','Ãƒâ€žÃ¢â‚¬Â¦'), 
										array('&#976;', '&#962;','&#286;', '&#287;', '&#305;', '&#304;', '&#350;', '&#351;','&#380;','&#379;','&#378;','&#377;','&#347;','&#346;','&#324;','&#323;','&#322;','&#321;','&#263;','&#262;','&#243;','&#211;','&#281;','&#280;','&#261;','&#260;'), 
										$html_preheader
										);
		$preheader 		= "<tr><td style='padding:0px; text-align: center; padding-bottom:15px;font-weight:bold;'>".$html_preheader."</td></tr>";
		/* End declaration text prÃƒÆ’Ã‚Â© header */
		$header = array();
		$afterObject ='';
		$footertext ='';
		$inc= "[EMV INCLUDE]";
	...
	public function actionMacro(){ 
		...
		if(Yii::app()->request->getParam('submitForm') !== NULL){
			$isacq = '';
			//RÃƒÆ’Ã‚Â©cupration des variables
			...
			//appelle fonction vue html
			///ajouter test macro.php sur les porteurs fids et acq
			$htmlData = $this ->actionGetHtmlCode($fid_border_color,  $fid_preheader,$fid_name,$porteur,$fid_product);
			$macroErrors = $htmlData['error'];
			// Lecture des fichiers uploader
			$excelfiles = CUploadedFile::getInstancesByName('macroFiles');
			...
			//max des contenues dynamiques ÃƒÆ’  crÃƒÆ’Ã‚Â©er
			$maxContenuDynamique=0;
			
			foreach($excelfiles as $macroFile => $pic){
				if (!$pic->saveAs(dirname(__FILE__).'/../../views/macro/macrosdocs/'.$pic->name)) {
					echo "ProblÃƒÆ’Ã‚Â©me lors de l'enregistrement du fichier";exit;
				}
				$filename = dirname(__FILE__).'/../../views/macro/macrosdocs/'.$pic->name;
				chmod($filename, 0777);
				...
				//definition du path du ficier avec le nom du fichier telechargÃƒÆ’Ã‚Â©

				//le fichier text avec extension TXT

				if (!copy(dirname(__FILE__).'/../../views/macro/macrosdocs/'.$pic->name, $filenamezip)){
					die("La copie du fichier a ÃƒÆ’Ã‚Â©chouÃƒÆ’Ã‚Â©...\n");
				}
				...
				if ($zip->open($filenamezip) === TRUE){
					...
					die("ProblÃƒÆ’Ã‚Â©me lors de l'extraction du fichier.");
				}

				//Traitement de la Macro: CrÃƒÆ’Ã‚Â©ation de l'url
				$macro = new \Business\Macro;
				...
				foreach($paragraphes as $para){
									...
									if ($rp->color) {
										switch ($rp->color['val']) {
											...
											case 'C0504D':
												...
												break;
											case '0070C0':
												if ($word->t=='[PavÃƒÆ’Ã‚Â© chÃƒÆ’Ã‚Â¨que]' || $word->t=='[PavÃƒÆ’Ã‚Â© check]' || $word->t=='[PavÃƒÆ’Ã‚Â© cheque]') {
													$word->t = '[EMV INCLUDE]'.$GLOBALS['porteurUrl'][$porteur]['emvchq'].'[EMV /INCLUDE]';
												}
												break;
											default:
					...
					$paratext = preg_replace('@<pn>(\s*)#([^\.;,]+)/([^\.,;]+)(\s*)</pn>@', "$1[EMV IF] (TITLE = \"1\") [EMV THEN]$2[EMV /THEN] [EMV ELSE]$3[EMV /ELSE] [EMV /IF]$4" , $paratext);
					$paratext = preg_replace('@<pn>(\s*)#([^\.;,/s]+)/([^\.,;/s]+)(\s*)</pn>@', "$1[EMV IF] (TITLE = \"1\") [EMV THEN]$2[EMV /THEN] [EMV ELSE]$3[EMV /ELSE] [EMV /IF]$4" , $paratext);
					$paratext = str_replace("# PrÃƒÆ’Ã‚Â©nom", $emvFName, $paratext);
					$paratext = str_replace("#PrÃƒÆ’Ã‚Â©nom", $emvFName, $paratext);
					$paratext = str_replace("# prÃƒÆ’Ã‚Â©nom", $emvFName, $paratext);
					$paratext = str_replace("#prÃƒÆ’Ã‚Â©nom", $emvFName, $paratext);
...
//					}
					...
					$paratext = preg_replace('@<lien>(.+)</lien>@', "$1 \n\t: [EMV LINK]1[EMV /LINK]", $paratext);
					$paratext = preg_replace('@<pr>(.*)#(.+)</pr>@', $GLOBALS['porteurUrl'][$porteur]['nom'], $paratext);
					//supprimer phrases aprÃƒÆ’Ã‚Â©s signature pour les les porteur ici
					if($porteur =='fr_rmay' || $porteur =='de_rmay' || $porteur =='es_rmay' || $porteur =='nl_rmay' || $porteur =='br_rmay' || $porteur =='pt_rmay' || $porteur =='pt_rmay' || $porteur =='pt_rmay' || $porteur =='tr_rmay' || $porteur =='dk_rmay' || $porteur =='no_ml' || $porteur =='ie_ml' || $porteur =='ie_ml'){
						$paratext = preg_replace('@<pr>(.*)</pr>@', '', $paratext);
					}

					//$paratext = str_replace(array('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦','ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢' ,'<pn>', '</pn>', '<vs>', '</vs>', '<vp>', '</vp>', '<pr>', '</pr>', '<lien>', '</lien>'), array('...','\'','','','','','','','','',''), $paratext);
...
//					}
					...
					$paratexthtml = preg_replace('@<pn>(\s*)#([^\.;,]+)/([^\.,;]+)(\s*)</pn>@', "$1[EMV IF] (TITLE = \"1\") [EMV THEN]$2[EMV /THEN] [EMV ELSE]$3[EMV /ELSE] [EMV /IF]$4" , $paratexthtml);
					$paratexthtml = preg_replace('@<pn>(\s*)#([^\.;,/s]+)/([^\.,;/s]+)(\s*)</pn>@', "$1[EMV IF] (TITLE = \"1\") [EMV THEN]$2[EMV /THEN] [EMV ELSE]$3[EMV /ELSE] [EMV /IF]$4" , $paratexthtml);
					$paratexthtml = str_replace("#PrÃƒÆ’Ã‚Â©nom", $emvFName, $paratexthtml);
					$paratexthtml = str_replace("# PrÃƒÆ’Ã‚Â©nom", $emvFName, $paratexthtml);
					$paratexthtml = str_replace("#prÃƒÆ’Ã‚Â©nom", $emvFName, $paratexthtml);
					$paratexthtml = str_replace("# prÃƒÆ’Ã‚Â©nom", $emvFName, $paratexthtml);
				...
				$allparahtml = str_replace(array('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦','ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢' ,'<pn>', '</pn>', '<vs>', '</vs>', '<vp>', '</vp>', '<pr>', '</pr>', '<lien>', '</lien>'), array('...','\'','','','','','','','','',''), $allparahtml);
				$allparatext = str_replace(array('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦','ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢' ,'<pn>', '</pn>', '<vs>', '</vs>', '<vp>', '</vp>', '<pr>', '</pr>', '<lien>', '</lien>'), array('...','\'','','','','','','','','',''), $allparatext);
				$textObjets[1] = str_replace(array('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦','ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢' ,'<pn>', '</pn>', '<vs>', '</vs>', '<vp>', '</vp>', '<pr>', '</pr>', '<lien>', '</lien>'), array('...','\'','','','','','','','','',''), $textObjets[1]);
				$parastext = str_replace(array('ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦','ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢' ,'<pn>', '</pn>', '<vs>', '</vs>', '<vp>', '</vp>', '<pr>', '</pr>', '<lien>', '</lien>'), array('...','\'','','','','','','','','',''), $parastext);
...
//					die($allparatext);
				...
				//replace spechial charactere for polish and turkish
				// $text = str_replace(array('Ãƒâ€¦Ã‚Â¼','Ãƒâ€¦Ã‚Â»' ,'Ãƒâ€¦Ã‚Âº', 'Ãƒâ€¦Ã‚Â¹', 'Ãƒâ€¦Ã¢â‚¬Âº', 'Ãƒâ€¦Ã…Â¡', 'Ãƒâ€¦Ã¢â‚¬Å¾', 'Ãƒâ€¦Ã†â€™', 'Ãƒâ€¦Ã¢â‚¬Å¡', 'Ãƒâ€¦Ã¯Â¿Â½', 'Ãƒâ€žÃ¢â‚¬Â¡', 'Ãƒâ€žÃ¢â‚¬Â ','ÃƒÆ’Ã‚Â³','ÃƒÆ’Ã¢â‚¬Å“','Ãƒâ€žÃ¢â€žÂ¢','Ãƒâ€žÃ‹Å“','Ãƒâ€žÃ¢â‚¬Â¦','Ãƒâ€žÃ¢â‚¬Å¾'), 
				// 				    array('&#380;','&#379;','&#378;','&#377;','&#347;','&#346;','&#324;','&#323;','&#322;','&#321;','&#263;','&#262;','&#243;','&#211;','&#281;','&#280;','&#261;','&#260;')
				// 		, $text);	
				$html = str_replace(array('Ãƒâ€¦Ã‚Â¼','Ãƒâ€¦Ã‚Â»' ,'Ãƒâ€¦Ã‚Âº', 'Ãƒâ€¦Ã‚Â¹', 'Ãƒâ€¦Ã¢â‚¬Âº', 'Ãƒâ€¦Ã…Â¡', 'Ãƒâ€¦Ã¢â‚¬Å¾', 'Ãƒâ€¦Ã†â€™', 'Ãƒâ€¦Ã¢â‚¬Å¡', 'Ãƒâ€¦Ã¯Â¿Â½', 'Ãƒâ€žÃ¢â‚¬Â¡', 'Ãƒâ€žÃ¢â‚¬Â ','ÃƒÆ’Ã‚Â³','ÃƒÆ’Ã¢â‚¬Å“','Ãƒâ€žÃ¢â€žÂ¢','Ãƒâ€žÃ‹Å“','Ãƒâ€žÃ¢â‚¬Â¦'), 
								    array('&#380;','&#379;','&#378;','&#377;','&#347;','&#346;','&#324;','&#323;','&#322;','&#321;','&#263;','&#262;','&#243;','&#211;','&#281;','&#280;','&#261;','&#260;')
						, $html);
...
/********************************************************************* GÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©ration des emails via l'API *******************************************************************************/
...
/********************************************* Connexion ÃƒÆ’  l'API ***********************************************/
		...
		'es_laetizia'	=> 'es',
				...
				// RÃƒÆ’Ã‚Â©cupere  le nom du porteur
				
				$porteur_name = $class_api->getNomPorteur($token);

				

				//rÃƒÆ’Ã‚Â©cuperer la langue du porteur
				$porteur_langue = $class_api->getLangue($token);
...
/******************************************************partie objet*****************************************/
                while( preg_match( '#\[J\+[0-9][0-9]?\]#' , $textObjets[1] ) ) 
				{
                    ...
                    if( !$exist )
					{
				   		...
				   		$contentD = "[EMV EXTERNAL][EMV CACHED]false[EMV /CACHED][EMV URLNAME]http://$urldynamique/$porteurMapp/fonctions_gen/dateComplet.php?rt=returnDt&sd=%2B$number&language=$porteur_langue&sdEmv=[EMV FIELD]EMVADMIN$fid_sd"."[EMV /FIELD][EMV /URLNAME][EMV /EXTERNAL]";
...
/*************************************************************************************************************/
				...
				// CrÃƒÆ’Ã‚Â©ation de l'email au niveau de SF
				$class_api->createMessageEmail($token,$fileNameZip[0],$textObjets[1],$porteur_name,$html2,$porteur_name,$mail,$isBounceback,$description,$encoding,$hotmailUnsubFlg,$hotmailUnsubUrl,$type,$macroURL,$FAQURL);



/********************************************************************* Fin GÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©ration des emails via l'API *******************************************************************************/
			...
			Yii::app()->user->setFlash( "success", "<b>Les fichiers sont gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rÃƒÆ’Ã‚Â©s dans le dossier : ".$this->portViewDir."generated/$porteur/$fid_name$ct</b>" );
			$this->render( '//product/macro', array('Router' => $Router, 'porteur' => $_SESSION['porteur']));
	...
	public function actionUploadImages(){
		...
		if(isset($imageToDelete) && !empty($imageToDelete)){
			...
			if (file_exists($imageToDelete)) {
				if (unlink($imageToDelete)) {
					$msg = "L'image $imageToDelete ÃƒÆ’  ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© supprimÃƒÆ’Ã‚Â©.";
				} else {
					$msg = "L'image $imageToDelete n'ÃƒÆ’  pas ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© supprimÃƒÆ’Ã‚Â©.";
				}
			}else{
		...
		//load product
		if( !($prod = \Business\Product::loadBySubCamp( $id )) )
			return false;
		...
		$viewDir 	= SERVER_ROOT.$this->portViewDir.$porteur;
		$prodir    = $viewDir.'/'.$camp->ref.'/'.$prod->ref;
		if (!file_exists($prodir)) {
			die("le dossier <b> $prodir </b> n'existe pas sur le serveur de production");
		}
...
////
	...
	/**
	 *  V1 dupplication view
	 *
	 * @author Mounir Mouih
	 * @return view
	 */
	public function ActionDupplicationV1()
	{
		// error_reporting(0);
		//global $porteurMap;
		$porteurMap = $GLOBALS['porteurMap'];
		// NOTE:  controle before/after save events
		define('EVENT_SAVE_ESCAPE', true);

		$porteur	= \Yii::app()->params['porteur'];

		if(Yii::app()->request->getParam( 'pr' ) !== null){
			$file	= Yii::app()->request->getParam( 'pr' );
		} else {
			$file =   $porteurMap[$porteur];
		}
		/* dupplication V1*/
		if (Yii::app()->request->getParam( 'dupplicate' ) !== null) {
			/* product refs map */
			$mapProduct =  array(
				'vg' => ["ref" => 'stc_1', 'name' => 'voygratuite', 'isCT'=>0],
				'vp' => ["ref" => 'stc_2', 'name' => 'voypayante', 'isCT'=>0],
				'in' => ["ref" => 'stc_3', 'name' => 'inter', 'isCT'=>0],
				'ct' => ["ref" => 'stc_4', 'name' => 'conttel', 'isCT'=>1],
				'as' => ["ref" => 'stc_5', 'name' => 'Asile', 'isCT'=>1],
				'as2' => ["ref" => 'stc_6', 'name' => 'Asile 2', 'isCT'=>1],
				'as3' => ["ref" => 'stc_7', 'name' => 'Asile 3', 'isCT'=>1],
				'as4' => ["ref" => 'stc_8', 'name' => 'Asile 4', 'isCT'=>1],
			);

			$file	    = trim(strip_tags($porteurMap[Yii::app()->request->getParam( 'porteur_key' )])) ;
			$views_file = str_replace('index.php', '', $_SERVER['SCRIPT_FILENAME'])."views/".trim(strip_tags($porteur)) ;
            //echo $views_file;
            //exit;
            // VÃƒÆ’Ã‚Â©rification de l'existence du dossier porteur dans le dossier views ( Ex: /_dev_yii_0929/views/pt_laetizia )
			if(!file_exists($views_file)){
				echo json_encode(array(
					"error" => " le dossier $views_file n&acute;existe pas !"
				));
				die;
			}
			//$this->loadConfigForPorteur( trim(strip_tags(Yii::app()->request->getParam( 'porteur_key' ))) );

            //RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration des fichiers selectionnÃƒÆ’Ã‚Â©s (Ex: /vgldv/html.php, /vgbdc_acq/html.php, /vgltalisman/document.php ... )
			$selections       = Yii::app()->request->getParam( 'selections' );
			//var_dump($selections);
			//exit;
			//RÃƒÆ’Ã‚Â©cupÃƒÆ’Ã‚Â©ration du porteur selectionnÃƒÆ’Ã‚Â©
			$selected_porteur = Yii::app()->request->getParam( 'porteur' );
			...
			if(!$selections){
				...
				$error_messages  = [];
				$notice_messages = [];
				// $Campaign = \Business\Campaign::loadByRef('stc');
				$Campaign = \Business\Campaign::model()->findAllBySql( "select * from V2_campaign where ref like 'stc'" );
				if( $Campaign ){
					/* REGEX conditions */
					$map =  array(
						'vg' => '(^vg$)|gratuit',
						...
						'in' => '(^in$)|inter',
						'ct' => 'conttel',
						'as' => '(^as$)|asil[^\d]*([a-z])(\s+\w+\d+)*$',
						'as2' => '(^as(_?)2)|asile\s*2|asil\s*2',
						'as3' => '(^as(_?)3)|asile\s*3|asil\s*3',
						'as4' => '(^as(_?)4)|asile\s*4|asil\s*4',
					);
					if(count($Campaign) > 1){
						$inf = [];
						...
						$inf = 'IDs : '.implode(' , ', $inf);
						$error_messages[] = "La campaign STC existe plus qu&acute;une fois ! ($inf)";
						goto error_message;
					} else {
						$Campaign =  $Campaign[0];
					}

					foreach ($selections as $product => $reflations) {
						//echo "<pre>";
						//print_r($map[$product]);
						/* product */
						$res  =  \Business\Product::loadByCriteria( $map[$product], ['ref', 'description', 'titleStat'] ) ;
						if ($res) {
							$infos = '';
							foreach ($res as $k => $item) {
								$infos .= "ID: $item->id, Ref: $item->ref, ";
							}
							if (count($res) > 1) {
								$error_messages[] = "Le produit $product existe plus qu&acute;une fois ! ($infos)";
							} else {
								$notice_messages[] = "Le produit $product existe deja ($infos)";
							}
							// NOTE: Campaign + product exists
							if (count($res) ==  1) {
								$subCampainID =  \Business\SubCampaign::loadByCampaignAndProduct($Campaign->id, $res[0]->id);
								//var_dump($subCampainID->attributes);
								//exit;
								$this->create_reflations_dupp_v1($views_file, $file, $Campaign, $subCampainID->id, $reflations, $res[0], $error_messages, $notice_messages);
							}

						} else {
							// NOTE: Campaign exists, product doesn't
							/* creating  product :  and reflations */

							$this->create_product_and_reflations_dupp_v1($views_file, $file, $Campaign, $reflations, $product, $mapProduct, $error_messages, $notice_messages);
						}

					}
					error_message:
					if (count($error_messages) > 0 || count($notice_messages) > 0  ) {
						$messages = [];
						if (count($error_messages) > 0) {
							$messages['error']  = "<b><u>Veuillez resoudre les problemes suivants avant de faire la dupplication :</u></b> <br><br> ".implode('<br><br>', $error_messages);
						} else {
							$messages['success'] = "Dupplication avec succ&egrave;s,";
						}
						if (count($notice_messages) > 0) {
							$messages['notice']  = "<b><u>warning :</u></b> <br><br> ".implode('<br><br>', $notice_messages);
						}
						echo json_encode($messages);
					} else {
						echo json_encode(['success' => "Dupplication avec succ&egrave;s."]);
					} 

					$this->loadConfigForPorteur( trim(strip_tags($porteur)) );
					die;
				} else {
					/* NOTE creating campaign, product and reflations */
					$Camp = new \Business\Campaign;
					$Camp->ref = "STC";
					$Camp->label = "Strat Centrale";
					if($Camp->save()){
						foreach ($selections as $product => $reflations) {
							$this->create_product_and_reflations_dupp_v1($views_file, $file, $Camp, $reflations, $product, $mapProduct, $error_messages, $notice_messages);
						}
					}
		...
		/*  connectors  */
		if (Yii::app()->request->getParam( 'connector' ) !== null) {
			$connector  = Yii::app()->request->getParam( 'connector' );
			$dir        = urldecode(Yii::app()->request->getParam('dir'));
			$target     = trim(Yii::app()->request->getParam('target'));
			// $viewDir 	= SERVER_ROOT.$this->portViewDir.$porteur;
			// $path 		= $this->portViewDir.$porteur;
			/* Root Directory */
			$root = "/home/www/www.spirit2010.com/$file";
			if($dir == '/'){
				echo("<pre style='background: #d13a2f; color: #fff; '>/{$file}</pre>");
			}
				...
				if( count($files) > 2 ) { /* The 2 accounts for . and .. */
					...
					foreach( $files as $file ) {
						if( file_exists($root . $dir . $file) && $file != '.' && $file != '..' && is_dir($root . $dir . $file) ) {
							if (preg_match("@^{$target}[^\d]|css@", $file) && !preg_match('@_pp|_pc|_old|\.old@', $file)) {
								echo "<li class=\"directory collapsed\">
										<a href=\"#\" rel=\"" . htmlentities($dir . $file) . "/\"data-target='".htmlentities($target)."'>".
											htmlentities($file)
										."</a></li>";
							}
						}
					...
					if($dir != '/'){
						foreach( $files as $file ) {
							if( file_exists($root . $dir . $file) && $file != '.' && $file != '..' && !is_dir($root . $dir . $file) ) {
								...
								// if (preg_match("@jpg|png|jpeg|css|php|js@i", $ext)) {
		...
		/* Template */
		$this->renderPartial( '//product/DupplicationV1', array(
			'porteurs' => $porteurMap,
			'porteur' => $porteur,
		));
	...
	/**
	 * create Products and Reflation files and References in the database
	 *
	 * @author Mounir
	 * @param Array reflations  reflation list
	 * @return void
	 */
	private function create_product_and_reflations_dupp_v1($views_file, $file, $Campaign, Array $reflations, $product, Array $mapProduct, Array &$error_messages , Array &$notice_messages){
		/* create product*/
		$productObject = new \Business\Product;
		$productObject->ref = $mapProduct[$product]['ref'] ;
		$productObject->description = $mapProduct[$product]['name'];
		$productObject->titleStat = $mapProduct[$product]['name'];
		$productObject->abrStat = $product;
		$productObject->isCT = $mapProduct[$product]['isCT'];
		
		
		//die($productObject->isCT);
		/* defaults*/
		$productObject->productType = 1;
		$productObject->productType2 = '';

		   		
		$productObject->qty = 1;
		$productObject->amountBif = '1.00';
		$productObject->priceModel = 'standard';
		$productObject->bdcFields = '{}';
		$productObject->priceModel = '';
		$productObject->paramPriceModel = '';
		$productObject->ctdate = '';
		$productObject->theoPricePros = '';
		$productObject->theoPriceVg = '';
		$productObject->theoPriceVp = '';
		$productObject->theoPriceCt = '';
		$productObject->asile_type = '';
		$productObject->produit_expirable = '';

		if($productObject->save()){
			/* product saved */
			$product_id = $productObject->id;
			 // NOTE: Creating sub_campaign
			 $sub_campaign = new \Business\SubCampaign;
			 $sub_campaign->idCampaign = $Campaign->id ;
			 $sub_campaign->position = explode('_', $mapProduct[$product]['ref'])[1] ;
			 $sub_campaign->idProduct = $product_id ;
			 if($sub_campaign->save()){
				/* campaign saved */
				/* Find the sub_campaign_id stored */
				$sub_campain_id =  $sub_campaign->id;
				// NOTE: create reflations, files + References (DB)
				$this->create_reflations_dupp_v1($views_file, $file, $Campaign, $sub_campain_id, $reflations, $productObject, $error_messages, $notice_messages);
			} else {
				$error_messages[] = "Une erreur du subCampagain du produit $product sur la base de donnees ";
				return;
			}
		} else {
			echo $productObject->getErrors(); die;
		}
	}

	/**
	 * create Reflation files and References in the database
	 *
	 * @author Mounir
	 * @param Array reflations  reflation list
	 * @return void
	 */
	private function create_reflations_dupp_v1($views_file, $file, $Campaign, $sub_campain_id, Array $reflations, $product, Array &$error_messages , Array &$notice_messages)
	{
		$product_id = (int) $product->id;
		$sub_campain_id = (int) $sub_campain_id;
		foreach ($reflations as $ref_source) {
			$views = [];
			/* list views for a given product*/
			if ($sub_campain_id) {
				$refls = \Business\SubCampaignReflation::model()->findAllBySql( "select * from V2_subcampaignreflation where idSubCampaign = $sub_campain_id" );
				foreach ($refls as  $refl) {
					$views[] = $refl->view;
				}
			}

			$reflation = explode('/', $ref_source)[1];
			$file_target = explode('/', $ref_source)[2];
			$root = "/home/www/www.spirit2010.com/$file";
			/* Get the data */
			$target  = $root.$ref_source;
			$data = file_get_contents($target);
/*echo($data);
exit;*/
			/* NOTE : create product and Campaign files if !exists */

			$camp = $Campaign->ref;
			$product_ref = $product->ref;

			$create_campaign  = file_exists("$views_file/$camp") ? false : mkdir("$views_file/$camp");
			$create_campaign ? chmod("$views_file/$camp", 0777) : NULL;

			$create_product   = file_exists("$views_file/$camp/$product_ref") ? false : mkdir("$views_file/$camp/$product_ref");
			$create_product ? chmod("$views_file/$camp/$product_ref", 0777) : NULL;

			if(preg_match("/css/", $reflation)){
				/* NOTE : fichiers css */
				$create_css_folder = file_exists("$views_file/$camp/$product_ref/css") ? false : mkdir("$views_file/$camp/$product_ref/css");
				$create_css_folder ? chmod("$views_file/$camp/$product_ref/css", 0777) : NULL;

				$import_css_file = file_exists("$views_file/$camp/$product_ref/css/$file_target") ? false : file_put_contents("$views_file/$camp/$product_ref/css/$file_target", $data);
				$import_css_file ? chmod("$views_file/$camp/$product_ref/css/$file_target", 0777) : NULL;

			} elseif(preg_match("/image/", $reflation)) {
				/* NOTE : Dossiers images  */
				$create_images_folder = file_exists("$views_file/$camp/$product_ref/images") ? false : mkdir("$views_file/$camp/$product_ref/images");
				$create_images_folder ? chmod("$views_file/$camp/$product_ref/images", 0777) : NULL;

				$import_images_file = file_exists("$views_file/$camp/$product_ref/images/$file_target") ? false : file_put_contents("$views_file/$camp/$product_ref/images/$file_target", $data);
				$import_images_file ? chmod("$views_file/$camp/$product_ref/images/$file_target", 0777) : NULL;
			} else {
				// NOTE: REFLATIONS
				$temp  = explode('_', $reflation) ;
				$reflation = isset($temp[0]) ? $temp[0] : $reflation;
				$reflation = preg_match('@l$@', $reflation) ? 'pro' : $reflation;
				// NOTE: Getting the reflation number
				//$number =  $reflation == 'pro' ? 111 : preg_match('/r\d$/', $reflation) ? ( (int) preg_replace('/^.+r(\d)$/', "$1", $reflation) + 1 ) : 1;
				$number =  $reflation == 'pro' ? 111 : (preg_match('/r\d$/', $reflation) ? ( (int) preg_replace('/^.+r(\d)$/', "$1", $reflation) + 1 ) : 1);
				if ($views && preg_match('/'.implode('|', $views).'/', $reflation)) {
					$error_messages[] = "la relance : $reflation existe deja !";
				} else {
					// NOTE: Insert reflations into the database
					$sub_campaign_reflation         = new \Business\SubCampaignReflation;
					$sub_campaign_reflation->number = $number;
					$sub_campaign_reflation->view   = $reflation;
					$sub_campaign_reflation->idSubCampaign  = $sub_campain_id;
					$sub_campaign_reflation->templateProd   = 'default_templateProduct';
					$sub_campaign_reflation->isDeliveryView = '0';
                    
                   // var_dump($sub_campaign_reflation->attributes);
                   // exit;
					if($sub_campaign_reflation->save()){
						$create_reflation = file_exists("$views_file/$camp/$product_ref/$reflation.html") ? NULL : file_put_contents("$views_file/$camp/$product_ref/$reflation.html", $data);
						$create_reflation ? chmod("$views_file/$camp/$product_ref/$reflation.html", 0777) : NULL;
						
						$tpl = file_get_contents("$views_file/../templateprod/".\Yii::app()->params['lang']."/default_templateProduct.html");
						file_put_contents("$views_file/$camp/$product_ref/default_templateProduct.html", $tpl);
						//chmod("$views_file/$camp/$product_ref/default_templateProduct.html", 0777);
						//chmod("$views_file/$camp/$product_ref/$reflation.html", 0666);
                       
						if ($create_reflation === NULL) {
							$error_messages[] = "La relance /$camp/$product_ref/$reflation.html existe deja! veuillez supprimer le fichier et reessayer";
						} elseif($create_reflation === false) {
							$error_messages[] = "La relance /$camp/$product_ref/$reflation.html n&acute;a pas du etre cree!";
						}
					} else {
						$error_messages[] = "La relance $reflation n&acute;a pas du etre cree dans la base de donnees!";
					}
				}
			}
		}
...
public function actionCampaignDuplicateOld(){
		...
		if( Yii::app()->request->getParam( 'id' ) !== NULL )
		{
			...
			if( !($SubCamp = \Business\SubCampaign::model()->findAllBySql( "select * from V2_subcampaign where idCampaign = $Camp->id" )))
			    return false;

			foreach ($SubCamp as $SubProduct) {

				$Product[] = $SubProduct->idProduct;
				$ref_product = \Business\Product::load($SubProduct->idProduct)->ref ;

				$SubCampRef = \Business\SubCampaignReflation::model()->findAllBySql( "select * from V2_subcampaignreflation where idSubCampaign = $SubProduct->id" );
				if($SubCampRef)
				{
					foreach ($SubCampRef as $Reflation) {
						$listeSelactions[$Camp->ref][$ref_product][$Reflation->id] = $Reflation->view;
					}
				}
			}
		}
		...
		if( Yii::app()->request->getParam( 'Business\Campaign' ) !== NULL )
		{
				$this->loadConfigForPorteur( trim(strip_tags(Yii::app()->request->getParam( 'porteur_target' ))) );

				//$selections = Yii::app()->request->getParam( 'selections' );
				$selected_porteur  = Yii::app()->request->getParam( 'porteur_target' );
				$selected_campaign = Yii::app()->request->getParam( 'reference_target' );
				$selected_informationCamp = json_decode(Yii::app()->request->getParam( 'informationCamp' ), true);
			
				if(!$selected_campaign){
					die;
				} else {
					// $Campaign = \Business\Campaign::loadByRef('stc');
					$Campaign = \Business\Campaign::model()->findAllBySql( "select * from V2_campaign where ref like '".$selected_campaign."'" );
						...
						if($Campaign){
							...
							$notice_messages[] = "La campaign ($selected_campaign) existe deja sur ($selected_porteur)! ($inf)";
							//goto error_message;

						} else {
						// Campaigne inexistante au niveau de porteur selectionÃƒÆ’Ã‚Â©


						}

						//echo "<pre>"; print_r($selected_informationCamp); echo "</pre>";die;

						foreach ( $selected_informationCamp as $camps => $prodsTab) {

							// echo "<pre>"; print_r(\Business\Product::loadByRef( $product )); echo "</pre>";die;
							/* product */
							$liste_produit = '';
							foreach ($prodsTab as $prod => $reflationsTab) {
									$liste_produit .= '('.$prod.')|';
								}

							$liste_produit = substr($liste_produit, 0, -1);
							$res  =  \Business\Product::loadByCriteria( $liste_produit, ['ref', 'description', 'titleStat'] ) ;
							if ($res) {
								$infos = '';
								foreach ($res as $k => $item) {
									$infos = "ID: $item->id, Ref: $item->ref, ";
									$error_messages[] = "Le produit ($item->ref) existe deja sur ($selected_porteur)! ($infos)";
								}
						...
						if (count($error_messages) > 0 || count($notice_messages) > 0  ) {
							$messages = [];
							if (count($error_messages) > 0) {
								$messages['error']  = "<b>Veuillez resoudre les probl&egrave;mes suivants avant de faire la dupplication :</b> <br><br> ".implode('<br><br>', $error_messages);
							}
							if (count($notice_messages) > 0) {
								$messages['notice']  = "<b><u>warning :</u></b> <br><br> ". implode('<br><br>', $notice_messages);
							}
							...
							/*create campagne and stuff */
							$idCampDup = Yii::app()->request->getParam( 'Business\Campaign' )['id'];
							/* creating campaign etc ... */
							$this->loadConfigForPorteur( trim(strip_tags($porteur)) );

							if( !($CampDup = \Business\Campaign::load($idCampDup)) )
								return false;

							$selected_porteur  = Yii::app()->request->getParam( 'porteur_target' );
							$selected_campaign = Yii::app()->request->getParam( 'reference_target' );
							$selected_label    = Yii::app()->request->getParam( 'label_target' );
							$selected_informationCamp = json_decode(Yii::app()->request->getParam( 'informationCamp' ), true);

							if( !($SubCampDup = \Business\SubCampaign::loadByCampaign($CampDup->id)) )
								return false;

							$tabProductObject = [];
							$tabPricingGrille = [];

							foreach ($selected_informationCamp as $CampSelect => $tabProd) {
								foreach ($tabProd as $Pro => $tabRf) {
									$ProductDup = \Business\Product::loadByref($Pro);
									$tabProductObject[] = $ProductDup;
									foreach ($tabRf as $idRef => $Rf) {
										$SubCampReflationDup = \Business\SubCampaignReflation::load($idRef);
										$tabSubCampReflationDup[] = $SubCampReflationDup;
									}

								}

								/* creating table grille tarifaire etc ... */

								$SubPriceDup = \Business\SubCampaign::loadByCampaignAndProduct($CampDup->id,$ProductDup->id);
								$idSite = 1;
								$SPricingGrilleDup = \Business\PricingGrid::getBySubCampaignAndSite($SubPriceDup->id,$idSite);
								foreach ($SPricingGrilleDup as $lignePGDup) {
										///echo "<pre>"; print_r($lignePGDup->id); echo "</pre>"; die;
									   $tabPricingGrille[] = $lignePGDup;
								}
							}
							...
							$idSite = 1;
							$SPricingGrilleDup = \Business\PricingGrid::getBySubCampaignAndSite($SubPriceDup->id,$idSite);
							$tabPricingGrille[] = $SPricingGrilleDup;*/		
							//echo "<pre>"; print_r($tabPricingGrille); echo "</pre>"; die;



							$this->loadConfigForPorteur( trim(strip_tags($selected_porteur)) );

							$CampaigneDup = new \Business\Campaign();

							$CampaigneDup->attributes = $CampDup->attributes;
							$CampaigneDup->ref        = $selected_campaign;
							$CampaigneDup->label      = $selected_label;

							//ajouter la campaine dans le porteur selectionÃƒÆ’Ã‚Â©
							$views_file = str_replace('index.php', '', $_SERVER['SCRIPT_FILENAME'])."views/".$selected_porteur ;

							$incr = 1;
							if( $CampaigneDup->save() )
							{
								// echo "<pre>"; print_r($_SERVER); echo "</pre>"; die;

								$create_campaign  = file_exists("$views_file/$CampaigneDup->ref") ? false : mkdir("$views_file/$CampaigneDup->ref");
								$create_campaign ? chmod("$views_file/$CampaigneDup->ref", 0777) : NULL;

								$ppSet = \Business\PaymentProcessorSet::model()->find( );

								foreach ($tabProductObject as $ObjectProd) {
									$ProductDup = new \Business\Product();
									$ProductDup->attributes = $ObjectProd->attributes; 
									$ProductDup->idPaymentProcessorSet = $ppSet->id; 
									$ProductDup->save();

									/* creating SubCampaig etc ... */
									$SubCampaignDup = new \Business\SubCampaign();
									$SubCampaignDup->position   = $incr;
									$SubCampaignDup->idCampaign = $CampaigneDup->id;
									$SubCampaignDup->idProduct = $ProductDup->id;
									$SubCampaignDup->save();

									$root = str_replace('index.php', '', $_SERVER['SCRIPT_FILENAME'])."views/$porteur/";//.$file
									$camp_source = Yii::app()->request->getParam( 'Business\Campaign' )['ref'];

									$target_prod  = $root."$camp_source/$ProductDup->ref/default_templateProduct.html";

									if(file_exists($target_prod)){

										$data = file_get_contents($target_prod);

										$create_reflation = file_exists("$views_file/$CampaigneDup->ref/$ProductDup->ref/default_templateProduct.html") ? NULL : file_put_contents("$views_file/$CampaigneDup->ref/$ProductDup->ref/default_templateProduct.html", $data);
										$create_reflation ? chmod("$views_file/$CampaigneDup->ref/$ProductDup->ref/default_templateProduct.html", 0777) : NULL;

										if ($create_reflation === NULL) {
											$koAjout_message[] = "La template /$CampaigneDup->ref/$ProductDup->ref/default_templateProduct.html existe d&egrave;ja! veuillez supprimer le fichier et r&eacute;essayer";
										} elseif($create_reflation === false) {
											$koAjout_message[] = "La template /$CampaigneDup->ref/$ProductDupProductDup->ref/default_templateProduct.html n&acute;a pas du etre cr&eacute;e!";
										}

										/* NOTE : Dossiers images  */
										$dir = "$camp_source/$ProductDup->ref/images/";
										...
										foreach( $files as $file ) {
											if( $file != '.' && $file != '..' && !is_dir($root . $dir . $file) ) {
												...
												$import_images_file = file_exists("$views_file/$CampaigneDup->ref/$ProductDup->ref/images/$file") ? false : file_put_contents("$views_file/$CampaigneDup->ref/$ProductDup->ref/images/$file", $data);
												$import_images_file ? chmod("$views_file/$CampaigneDup->ref/$ProductDup->ref/images/$file", 0777) : NULL;


											}
										}
										/* NOTE : Dossiers CSS  */
										$dir = "$camp_source/$ProductDup->ref/css/";
										...
										foreach( $files as $file ) {
											if( $file != '.' && $file != '..' && !is_dir($root . $dir . $file) ) {
												$data  = file_get_contents("{$root}{$dir}{$file}");
												$import_css_file = file_exists("$views_file/$CampaigneDup->ref/$ProductDup->ref/css/$file") ? false : file_put_contents("$views_file/$CampaigneDup->ref/$ProductDup->ref/css/$file", $data);
												$import_css_file ? chmod("$views_file/$CampaigneDup->ref/$ProductDup->ref/css/$file", 0777) : NULL;
										...
										$koAjout_message[] = "La template /$CampaigneDup->ref/$ProductDup->ref/ n&acute;existe pas dans le fichier source!";
									}
									$tab_ref_duplicate= [];
									foreach ($tabSubCampReflationDup as $ObjectCampReflationDup) {

										$CampReflationDup = new \Business\SubCampaignReflation();
										$CampReflationDup->attributes    = $ObjectCampReflationDup->attributes;
										$CampReflationDup->idSubCampaign = $SubCampaignDup->id;
										$CampReflationDup->save();

										$reflation = $ObjectCampReflationDup->view;
										//$file_target = explode('/', $ref_source)[2];
										/* Get the data */
										$target  = $root."$camp_source/$ProductDup->ref/".$reflation.".html";

										if(!in_array($reflation, $tab_ref_duplicate)){

											if(file_exists($target)){

												$data = file_get_contents($target);
												
												$create_reflation = file_exists("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html") ? NULL : file_put_contents("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html", $data);
												$messages['debug'] = [
													'chemin'			=> "$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html",
													'create_reflation' 	=> $create_reflation,
													'file_exist'		=> file_exists("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html"),
													'target'			=> $target,
													'reflation'			=> $reflation,
													//'tab_ref_duplicate' => $tab_ref_duplicate
												];
												$create_reflation ? chmod("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html", 0777) : NULL;


												if ($create_reflation === NULL) {
													$koAjout_message[] = "La relance /$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html existe d&egrave;ja! veuillez supprimer le fichier et r&eacute;essayer";
												} elseif($create_reflation === false) {
													$koAjout_message[] = "La relance /$CampaigneDup->ref/$ProductDupProductDup->ref/$ObjectCampReflationDup->view.html n&acute;a pas du etre cr&eacute;e!";
												}

											}else{
												$koAjout_message[] = "La relance /$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html n&acute;existe pas dans le fichier source!";
											}
											$tab_ref_duplicate[] = $reflation;
										//echo "<pre>"; print_r($CampReflationDup->getErrors()); echo "</pre>";die;
										}

									}

									foreach ($tabPricingGrille as $ObjectPricingGridDup) {

								    	if(is_object($ObjectPricingGridDup) && $ObjectPricingGridDup->id !== null){

											$CampPricingGridDup = new \Business\PricingGrid();
											$CampPricingGridDup->attributes      = $ObjectPricingGridDup->attributes;
											$CampPricingGridDup->idSubCampaign   = $SubCampaignDup->id;
											$CampPricingGridDup->prixTheorique   = $ObjectPricingGridDup->prixTheorique;
											
											$porteurSite = explode("_", $selected_porteur)[0];
											$porteurSite = strtolower($porteurSite) == 'en'?'in':$porteurSite;

											$site = \Business\Site::loadByCode( $porteurSite );

											$CampPricingGridDup->idSite   = $site->id;
											
											//echo "<pre>"; print_r($CampPricingGridDup->attributes); echo "</pre>";die;

											if(!$CampPricingGridDup->save()){
												echo "<pre>"; 
												print_r($CampPricingGridDup->getErrors()); 
												echo "</pre>";
												die;
											}
										}
								...
								$this->loadConfigForPorteur( trim(strip_tags($porteur)) );
								$flash_messages[] = "Ajout de la campagne ($selected_campaign) au porteur ($selected_porteur)";
							}else{
								$this->loadConfigForPorteur( trim(strip_tags($porteur)) );
								$koAjout_message[] = "Probleme d ajout de la campaigne ($selected_campaign) au porteur ($selected_porteur)";
							}
						}

						if (count($flash_messages) > 0 || count($koAjout_message) > 0  ) {
							//$messages = [];
							if (count($koAjout_message) > 0) {
								$messages['koAjout']  = "<b>Erreur :</b> <br><br> ".implode('<br><br>', $koAjout_message);
							}
							if (count($flash_messages) > 0) {
								$messages['flash']  = "<b><u>Ajout effectue avec succes :</u></b> <br><br> ". implode('<br><br>', $flash_messages);
							}
							#print_r($messages);die;
							echo json_encode($messages);
						}
		...
		$this->renderPartial( '//product/campaignDuplicate', array( 'Camp' => $Camp,'informationCamp' => $listeSelactions, 'Porteurs' => $porteurMap, 'porteur' => $porteur  ) );
	}

public function actionCampaignDuplicate(){
		define('EVENT_SAVE_ESCAPE', true);
							...
							if( $CampaigneDup->save() )
							{
								...
								foreach ($tabProductObject as $ObjectProd) {
									...
									foreach ($tabSubCampReflationDup as $ObjectCampReflationDup) {
										...
										/* Get the data */
										$target  = $root."$camp_source/$ProductDup->ref/".$reflation.".html";
										//$messages['debug0'] = $target;
										if(!in_array($reflation, $tab_ref_duplicate)){

											if(file_exists($target)){

												$data = file_get_contents($target);
												//$rfaray = scandir("$views_file/$CampaigneDup->ref/$ProductDup->ref");

												//$create_reflation = in_array("$ObjectCampReflationDup->view.html", $rfaray) ? NULL : file_put_contents("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html", $data);
												$create_reflation = file_exists("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html") ? NULL : file_put_contents("$views_file/$CampaigneDup->ref/$ProductDup->ref/$ObjectCampReflationDup->view.html", $data);
    ...
    public function actionloadBySubcampaignAndMessageNumber( $idSubCampaign, $number)
    {
        ...
        echo "<pre>";
        print_r(json_encode($SubCampReflation->attributes));

	}


    public function actionloadByDateAndIndice($indice=26)
    {
        $user = new Business\User;
        $users = $user->loadByDateAndIndice($indice);
        echo "<pre>";
        foreach ($users as $use){
        	print_r(json_encode($use));
		}
        echo "</pre>";
    }
