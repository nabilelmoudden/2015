	...
	public function actionExcel_v2()
	{
		...
		if(isset($_POST['tblexcel']) OR 1){
			...
			$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('D8', 'N°Fid')
				->setCellValue('E8', 'Label')
				->setCellValue('F8', 'Type projet')
				->setCellValue('G8', 'Date Shoot')
				->setCellValue('H8', 'Date réception CDC')
				->setCellValue('I8', 'Validation du CDC')
				//->setCellValue('J8', 'Lancement IT')
				->setCellValue('J8', 'Fin intégration IT')
				->setCellValue('K8', 'Livraison Qualité')
				->setCellValue('L8', 'Livraison Marketing')
				->setCellValue('M8', 'Commentaire');

			$objPHPExcel->getActiveSheet()->getStyle('B8:M8')->getFont()->setBold(true)
				->getColor()
				->setRGB('FFFFFF');
			...
			$objPHPExcel->getActiveSheet()->setAutoFilter('C8:K'.$rowID);
		}
		else
			Yii::app()->user->setFlash( "error", Yii::t( 'common', 'Erreur lors de la récupération du tableau à exporter' ) );
...
////
	...
	public function ActionDupplicationV1()
	{
		...
		if (Yii::app()->request->getParam( 'dupplicate' ) !== null) {
			...
			$file	= trim(strip_tags(Yii::app()->request->getParam( 'porteur_key' ))) ;
			$views_file = str_replace('index.php', '', $_SERVER['SCRIPT_FILENAME'])."views/".trim(strip_tags(Yii::app()->request->getParam( 'porteur_key' ))) ;
		...
		if( Yii::app()->request->getParam( 'Business\Campaign' ) !== NULL )
		{
				...
				if(!$selected_campaign){
					...
					$Campaign = \Business\Campaign::model()->findAllBySql( "select * from V2_campaign where ref like '".$selected_campaign."'" );
						...
						// Campaigne inexistante au niveau de porteur selectioné
						...
						if (count($error_messages) > 0 || count($notice_messages) > 0  ) {
							...
							//ajouter la campaine dans le porteur selectioné
