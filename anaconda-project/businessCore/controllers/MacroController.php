...
class MacroController extends AdminController
{
	...
	public function actionExcel_v2()
	{
		...
		$suivi_plan = $Camp->SuiviPlanification_v2();
...
//Badr Yassine -- passer a la lecture du fichier
	...
	public function actionProductShow(){
		...
		if( Yii::app()->request->getParam( 'Business\Product' ) !== NULL )
		{
			...
			$Prod->paramPriceModel		= isset($paramPriceModel[$Prod->priceModel]) ? $paramPriceModel[$Prod->priceModel] : NULL;
			if( $Sub->idProduct > 0 )
			{
...
// ====================================================================================================
	...
	public function actionCampaignShowMark(){
		...
		if( $id !== NULL && ! empty($id) )
		{
			...
			if(isset($SubC[2 - $nbprod]['idProduct'])){
				if($SubC[2 - $nbprod]['idProduct'] > 0){
					$idpro = $SubC[2 - $nbprod]['idProduct'];
					$idsubcamp = $SubC[2 - $nbprod]['id'];
				}
			}
		...
		/** Mise a jour ou création ::  Handle Request POST */
		if( Yii::app()->request->getParam( 'Business\Campaign' ) !== NULL ){
			// Log l'action courante
			$this->logAction( new CEvent($this, array( 'action' => \Business\Log::ACTION_ADMIN)));
		...
		if( Yii::app()->request->getParam( 'step' ) == '1' ){
			if($idpro > 0){
				$this->redirect( array('product/productShowMark', 'idpro'=>$idpro, /*'id' => $idsubcamp,*/ 'idCamp'=>$Camp->id, 'nbprod'=>$nbprod) );
			}else{
				$this->redirect( array('product/productShowMark', 'idCamp'=>$Camp->id, 'nbprod'=>$nbprod) );
			}
	...
	public function actionProductShowMark(){
		...
		$maxprod = Yii::app()->session['maxprod'];
		$maxprod = $maxprod - 1;
		...
		if( Yii::app()->request->getParam( 'id' ) > 0 ){
			...
			$Prod	= $Sub->Product;
		}elseif( Yii::app()->request->getParam( 'idCamp' ) !== NULL ){// ******************************************** Création d'un produit * //
			...
			if($nbprod >= 0){
				if(count($SubC) > $maxprod - $nbprod){
					if( isset($SubC[$maxprod - $nbprod]['idProduct']) ){
						if($SubC[$maxprod - $nbprod]['idProduct'] > 0)
							$idpro = $SubC[$maxprod - $nbprod]['idProduct'];
						$idsubcamp = $SubC[$maxprod - $nbprod]['id'];
					}
				}
			}

			if($idpro > 0){
				//if( !($Sub = \Business\SubCampaign::load( $idpro )) )
		...
		if( Yii::app()->request->getParam( 'step' ) != NULL){
			$this->redirect( array('product/pricingGridMark', 'id'=>$Sub->id, 'idCamp'=>$idcampm, 'nbprod'=>$nbprod, 'priceStep'=>$priceStep) );
		}else{
			$this->renderPartial( '//product/productShowMark', array( 'Sub' => $Sub, 'Prod' => $Prod, 'lPPSet' => $PPSet->findAll(), 'lSub' => $lSub ) );
		}
	...
	public function actionPricingGridMark(){
		...
		$tab[ $sites->country ] = array(
			'id'	=> $sites->id,
			'ajax'	=> $this->createAbsoluteUrl( '//product/getPricingGridMark', array( 'idSite' =>  $sites->id, 'idSub' => $Sub->id, 'parite' => $parite->id_parite, 'priceStep' => $priceStep, 'idCamp'=>$idCamp, 'nbprod'=>$nbprod, 'nbsites'=>1 ) )
		);
		...
		if( Yii::app()->request->getParam( 'step' ) == '3' && $nbprod > 0 )
			$this->redirect( array('product/productShowMark', 'idCamp'=>$idCamp, 'nbprod'=>$nbprod) );
		else{
			

			$this->renderPartial( '//product/pricingGridMark', array( 'Sub' => $Sub, 'tab' => $tab, 'idCamp'=>$idCamp, 'nbprod'=>$nbprod, 'nbsites'=>1 ) );
		}
	...
	public function actionGetPricingGridMark(){
		...
		$this->renderPartial( '//product/getPricingGrid_Mark', array( 'Sub' => $Sub, 'idSite' => $idSite, 'parite' => $parite->parite, 'data' => $data, 'data2' => $data2, 'nbRelance' => $nbRelance, 'nbGP' => MAX_GP, 'nbBS' => MAX_BS, 'idCamp'=>$idCamp, 'nbprod'=>$nbprod, 'nbsites'=>1) );
...
// ******************************************** Action Generation CDC * //
	public function actionGetCDCdMark() {
		...
		if(isset($subcs[0]->Product)){
			...
			$desc1 = $subcs[0]->Product->titleStat;
...
////////code html pour macro
	...
	public function actionMacro(){
		...
		Yii::import( 'ext.MacroTabsRatio', true );
		$Router = new \Business\RouterEMV('search');
