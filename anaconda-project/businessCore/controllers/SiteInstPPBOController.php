...
/**
 * Description of SiteInstPPBOController
 *
 * @author Youssef HARRATI
 * @package Controllers
 */
class SiteInstPPBOController extends AdminController {
    ...
    public function actionIndex() {
		...
		$this->redirect( 'siteInstPPBO/pageList');
    }
	...
	public function actionPageList() {
		...
		$viewDir = $this->adminViewDir.'/siteInstPPBO/';
		
		
    	$this->render( '//siteInstPPBO/pageList', array('porteur' => $porteur, 'viewDir' => $viewDir, 'baseDir' => \Yii::app()->baseUrl, 'site' => $site) );
    }
	...
	public function actionPageEdit() {
		...
		$porteur			= \Yii::app()->params['porteur'];
		$viewDir = SERVER_ROOT.$this->portViewDir.$porteur.'/siteInstPP/';
		$adminViewDir = $this->adminViewDir.'/siteInstPPBO/';
		...
		if(Yii::app()->request->getParam( 'id' ) !== NULL){
			...
			if(Yii::app()->request->getParam( 'content' ) !== NULL){
				...
				fclose($handle);
				Yii::app()->user->setFlash( "success", Yii::t( 'common', 'la modification est enregistrée dans le fichier: <b>'.$this->portViewDir.$porteur.'/siteInstPP/'.$file.'</b>' ) );
			}
		}
		$filePath = $this->portViewDir.$porteur.'/siteInstPP/'.$file;
		$file = $viewDir.$file;
		$contents = "";
		if(is_file($file)){
			$handle = fopen($file, "r");
		...
		if(Yii::app()->request->getParam( 'popup' ) == 1){
			if(Yii::app()->request->getParam( 'content' ) !== NULL){
				$this->redirect(array('SiteInstPPBO/pageList', 'site' => $site));
			}
			$this->renderPartial( '//siteInstPPBO/pageEditPartial', array('file' => $filePath, 'page' => $page, 'porteur' => $porteur, 'title' => $title, 'viewDir' => $viewDir, 'adminViewDir' => $adminViewDir, 'baseDir' => \Yii::app()->baseUrl, 'contents' => htmlspecialchars($contents), 'site' => $site) );
			
		}else
		{$this->render( '//siteInstPPBO/pageEdit', array('file' => $filePath, 'page' => $page, 'porteur' => $porteur, 'title' => $title, 'viewDir' => $viewDir, 'adminViewDir' => $adminViewDir, 'baseDir' => \Yii::app()->baseUrl, 'contents' => htmlspecialchars($contents), 'site' => $site) );}
    }
